...
const adminSectionIds = new Set(["dashboard", "analytics", "catalog", "add-product", "edit-product", "categories", "category-editor", "tax-categories", "tax-editor", "orders", "returns-refunds", "customers", "partners", "partner-packages", "partner-withdrawals", "partner-details", "sellers", "resellers", "seller-withdrawals", "seller-products", "reviews", "staff", "create-staff", "support-tickets", "banners", "blog", "blog-create", "pages", "page-editor", "footer", "marketing", "team", ...settingsSectionIds]);
  ...
  return (
    <div className="appShell berryWorkspace berryWorkspace--admin" style={{ "--admin-button-color": state.storefrontSettings.adminButtonColor || "#1e88e5" }}>
      ...
      <main>
        ...
        {active === "staff" && <Team mode="staff" onAdd={() => navigateAdmin("create-staff")} onManageAccess={() => navigateAdmin("team")} />}
...
function Team({ mode = "access", onAdd, onBack, onManageAccess }) {
  ...
  const [staffForm, setStaffForm] = useState(blankStaff); const [teamForm, setTeamForm] = useState({ name: "", code: "", leader: "", members: [] }); const [assignment, setAssignment] = useState({ entityType: "Seller", entity: "", action: "kyc", permissions: ["VIEW"], team: "", staff: "" }); const [assignmentEntities, setAssignmentEntities] = useState([]); const [notice, setNotice] = useState("");
  const load = async () => {
  ...
  useEffect(() => { if (mode !== "access") return; const loaders = { Seller: () => api.adminSellers({ page: 1, limit: 500 }), Partner: () => api.adminPartners({ page: 1, limit: 500 }), Customer: () => api.customers({ page: 1, limit: 500 }), Reseller: () => api.adminResellers() }; setAssignmentEntities([]); loaders[assignment.entityType]().then(result => setAssignmentEntities(Array.isArray(result) ? result : result.items || result.customers || result.partners || [])).catch(error => setNotice(error.message)); }, [mode, assignment.entityType]);
  useEffect(() => {
