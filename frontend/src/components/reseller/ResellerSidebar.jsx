...
export default function ResellerSidebar({ pendingOrderCount = 0, menuOpen, setMenuOpen, branding, navItems, view, openAddFlow, setView, orders, onBack, logout }) {
  return (<aside id="reseller-navigation" className={`resellerSidebar ${menuOpen ? "isOpen" : ""}`}><button className="resellerDrawerClose" aria-label="Close navigation" onClick={() => { setMenuOpen(false); document.querySelector(".resellerMenuButton")?.focus() }}><X /></button>
    ...
    <nav aria-label="Reseller menu">{navItems.map(([key, label, Icon]) => <button key={key} aria-current={view === key ? "page" : undefined} className={view === key ? "active" : ""} onClick={() => key === "add" ? openAddFlow() : setView(key)}><Icon /><span>{label}</span>{key === "orders" && pendingOrderCount > 0 && <b aria-label={`${pendingOrderCount} pending orders`}>{pendingOrderCount}</b>}</button>)}</nav>
