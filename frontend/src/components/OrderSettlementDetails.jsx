...
export default function OrderSettlementDetails({ order, item, settlement, onClose, adminView = false }) {
  ...
  return <div className="modalOverlay settlementOverlay" role="dialog" aria-modal="true"><section className="orderSettlementDetails">
    ...
    <Block icon={Package} number="1" title="Order Value" totalLabel="Total Order Received" total={gross}><Row label="Total Order Value" value={totalOrderValue} /><Row label="COD Charges" value={customerCodCharge} deduction /><Row label="Shipping Charges" value={customerShipping} deduction /></Block>
    ...
    <section className="settlementGrandTotal"><Row label="Total Order Received" value={gross} /><Row label="Total Deductions" value={deductions} deduction /><div><span>Net Seller Settlement</span><strong>{money(settlement.netAmount)}</strong></div></section>
