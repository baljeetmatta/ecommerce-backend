import { CalendarDays, CircleDollarSign, Clock3, Package, SlidersHorizontal, Tag, Truck, WalletCards } from "lucide-react";
...
export default function OrderSettlementDetails({ order, item, settlement, onClose, adminView = false }) {
  ...
  const customerShipping = settlement.shippingPaidBy === "customer" ? Number(item.shippingCharge || 0) * Number(item.quantity || 1) : 0;
  ...
  const logistics = (settlement.shippingPaidBy === "seller" ? Number(settlement.shippingCharge || 0) : 0) + Number(settlement.returnRtoCharge || 0) + Number(settlement.codCharge || 0);
  const adjustments = Number(settlement.otherCharges || 0);
  const deductions = orderCharges + logistics + adjustments;
  ...
  return <div className="modalOverlay settlementOverlay" role="dialog" aria-modal="true"><section className="orderSettlementDetails">
    ...
    <section className="settlementProduct">{image ? <img src={image} alt="" /> : <i><Package /></i>}<div><h3>{item.name}</h3><p>Qty: {item.quantity} <b>|</b> SKU: {item.sku || "—"} <em>{item.seller?.isGstRegistered ? "GST SELLER" : "NON-GST SELLER"}</em></p>{adminView && item.seller && <small>Seller: {item.seller.companyName} ({item.seller.sellerNumber})</small>}</div></section>
    <Block icon={Package} number="1" title="Order Value" totalLabel="Gross Order Value" total={gross + customerShipping}><Row label="Product Selling Price" value={gross} /><Row label="Customer Discount" value={0} deduction /><Row label="Shipping Charged to Customer" value={customerShipping} positive /></Block>
    <Block icon={Tag} number="2" title="Platform & Payment Charges" totalLabel="Total Platform & Payment Charges" total={orderCharges} deduction><Row label={`Platform Fee (${settlement.commissionRate || 0}%)`} value={settlement.commissionAmount || settlement.platformFee} deduction /><Row label="GST on Platform Fee" value={settlement.gstOnCommission} deduction /><Row label={`Payment Gateway Fee (${settlement.paymentGatewayFeeRate || 0}%)`} value={settlement.paymentGatewayFee} deduction /><Row label="GST on Payment Gateway Fee (18%)" value={settlement.paymentGatewayGst} deduction /></Block>
    <Block icon={Truck} number="3" title="Shipping & Logistics" totalLabel="Total Shipping & Logistics" total={logistics} deduction><Row label={`Forward Shipping (${settlement.shippingPaidBy === "seller" ? "Seller" : "Customer / Admin"})`} value={settlement.shippingPaidBy === "seller" ? settlement.shippingCharge : 0} deduction /><Row label="Reverse Shipping / Return Pickup" value={settlement.returnRtoCharge} deduction /><Row label="COD / RTO Shipping" value={settlement.codCharge} deduction /></Block>
    <Block icon={SlidersHorizontal} number="4" title="Other Adjustments" totalLabel="Total Other Adjustments" total={adjustments} deduction><Row label="Advertisement Charge" value={0} deduction /><Row label="Other Adjustments" value={settlement.otherCharges} deduction /></Block>
    <section className="settlementGrandTotal"><Row label="Gross Order Value" value={gross + customerShipping} /><Row label="Total Deductions" value={deductions} deduction /><div><span>Net Seller Settlement</span><strong>{money(settlement.netAmount)}</strong></div></section>
