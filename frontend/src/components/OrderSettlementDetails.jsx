...
export default function OrderSettlementDetails({ order, item, settlement, onClose, adminView = false, sellerShippingMode }) {
  ...
  const customerCodCharge = order.codChargePaidBy === "customer" || item.codChargePaidBy === "customer" ? Number(order.codCharge || 0) : 0;
  ...
  return <div className="modalOverlay settlementOverlay" role="dialog" aria-modal="true"><section className="orderSettlementDetails">
    ...
    <Block icon={Package} number="1" title="Order Value" totalLabel="Total Order Received" total={gross}><Row label="Total Order Value" value={totalOrderValue} /><Row label="COD Charges" value={customerCodCharge} deduction />{!selfShipping && <Row label="Shipping Charges" value={customerShipping} deduction />}</Block>
