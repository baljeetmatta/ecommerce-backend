...
export default function OrderSettlementDetails({ order, item, settlement, onClose, adminView = false, sellerShippingMode }) {
  ...
  const customerShipping = !selfShipping && settlement.shippingPaidBy === "customer" ? Number(settlement.customerPaidShipping ?? (Number(item.shippingCharge || 0) * Number(item.quantity || 1))) : 0;
  ...
  return <div className="modalOverlay settlementOverlay" role="dialog" aria-modal="true"><section className="orderSettlementDetails">
    ...
    <Block icon={Package} number="1" title="Order Value" totalLabel="Total Order Received" total={gross}><Row label="Total Order Value" value={totalOrderValue} />{!selfShipping && <Row label="COD Charges" value={customerCodCharge} deduction />}{!selfShipping && <Row label="Shipping Charges" value={customerShipping} deduction />}</Block>
    ...
    <section className="settlementGrandTotal"><Row label="Total Order Received" value={gross} /><Row label="Total Deductions" value={Math.abs(deductions)} positive={deductions < 0} deduction={deductions >= 0} /><div><span>Net Seller Settlement</span><strong>{money(settlement.netAmount)}</strong></div></section>
