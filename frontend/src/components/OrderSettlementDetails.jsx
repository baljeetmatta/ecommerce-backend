...
function Block({ icon: Icon, number, title, children, totalLabel, total, deduction = false }) {
  return <section className="settlementBlock"><h3><Icon /> {number}. {title}</h3><div className="settlementRows">{children}</div><div className={`settlementBlockTotal ${deduction ? "deduction" : ""}`}><span>{totalLabel}</span><strong>{deduction ? (total < 0 ? "+ " : "− ") : ""}{money(deduction ? Math.abs(total) : total)}</strong></div></section>;
}
...
export default function OrderSettlementDetails({ order, item, settlement, onClose, adminView = false, sellerShippingMode }) {
  ...
  const customerShipping = settlement.shippingPaidBy === "customer" ? Number(settlement.customerPaidShipping ?? (Number(item.shippingCharge || 0) * Number(item.quantity || 1))) : 0;
  const customerCodCharge = !selfShipping && (order.codChargePaidBy === "customer" || item.codChargePaidBy === "customer") ? Number(order.codCharge || 0) : 0;
  const totalOrderValue = gross + customerShipping + customerCodCharge;
  ...
  return <div className="modalOverlay settlementOverlay" role="dialog" aria-modal="true"><section className="orderSettlementDetails">
    ...
    <Block icon={Package} number="1" title="Order Value" totalLabel="Total Order Received" total={gross + (selfShipping ? customerShipping : 0)}><Row label="Total Order Value" value={totalOrderValue} />{customerCodCharge > 0 && <Row label="COD Charges" value={customerCodCharge} deduction />}{customerShipping > 0 && <Row label="Shipping Charges" value={customerShipping} deduction={!selfShipping} positive={selfShipping} />}</Block>
    <Block icon={Tag} number="2" title="Platform & Payment Charges" totalLabel="Total Platform & Payment Charges" total={orderCharges} deduction><Row label={`Platform Fee (${settlement.commissionRate || 0}%)`} value={settlement.commissionAmount || settlement.platformFee} deduction /><Row label="GST on Platform Fee" value={settlement.gstOnCommission} deduction />{order.payment?.provider !== "cod" && <Row label={`Payment Gateway Fee (${settlement.paymentGatewayFeeRate ?? 0}% of order value)`} value={settlement.paymentGatewayFee} deduction />}{order.payment?.provider !== "cod" && <Row label="GST (18%) on Payment Gateway Fee" value={settlement.paymentGatewayGst} deduction />}</Block>
    {!selfShipping && <Block icon={Truck} number="3" title="Shipping & Logistics" totalLabel="Total Shipping & Logistics" total={logistics} deduction><Row label={settlement.shippingPaidBy === "seller" ? "Forward Shipping (Seller)" : shippingDeduction < 0 ? "ShipRocket Shipping Surplus" : shippingDeduction > 0 ? "ShipRocket Shipping Balance" : "Forward Shipping (Customer / Admin)"} value={Math.abs(shippingDeduction)} positive={shippingDeduction < 0} deduction={shippingDeduction >= 0} /><Row label="Reverse Shipping / Return Pickup" value={settlement.returnRtoCharge} deduction /><Row label="COD / RTO Shipping" value={settlement.codCharge} deduction /></Block>}
    <section className="settlementGrandTotal"><Row label="Total Order Received" value={gross + (selfShipping ? customerShipping : 0)} /><Row label="Total Deductions" value={Math.abs(deductions)} positive={deductions < 0} deduction={deductions >= 0} /><div><span>Net Seller Settlement</span><strong>{money(settlement.netAmount)}</strong></div></section>
