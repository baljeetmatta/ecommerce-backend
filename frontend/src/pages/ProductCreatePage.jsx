...
export default function ProductCreatePage({ categories, taxCategories, sellerSettlement = {}, gstDetails = null, products = [], initialProduct, onSave, onBack, hideCostPrice = false, hideStatus = false, gstEnabled = true, sellerShippingMode = "shiprocket" }) {
  ...
  useEffect(() => {
    ...
    if (sellerShippingMode !== "self" && ["free_realtime", "estimated_seller"].includes(next.shippingMode)) {
    ...
    <section className="contentStack">
      <form className="panel productForm" onSubmit={submitProduct}>
        ...
        <details className="productFormSection" open>
          ...
          <label><span>Customer Shipping</span><select value={form.shippingMode || "free_included"} onChange={(event) => { const mode = event.target.value; const customerPays = ["fixed_customer", "realtime_customer"].includes(mode); setForm((current) => ({ ...current, shippingMode: mode, shippingIncludedInPrice: !customerPays, shippingPaidBy: customerPays ? "customer" : "seller", shippingCharge: mode === "fixed_customer" ? current.shippingCharge : 0, shippingCost: sellerShippingMode === "self" ? 0 : current.shippingCost })); }}><option value="free_included">1. Free Shipping</option>{sellerShippingMode === "self" ? <option value="estimated_seller">2. Shipping charges paid by Seller</option> : null}<option value="fixed_customer">{sellerShippingMode === "self" ? "3. Fixed Shipping charges by Customer" : "2. Fixed Shipping by Customer"}</option>{sellerShippingMode !== "self" && <option value="realtime_customer">3. Real-time Shipping charged to Customer</option>}</select><small>{({ free_included: sellerShippingMode === "self" ? "Customer sees Free Shipping; seller handles delivery with no shipping deduction in settlement." : "Customer gets free shipping; the live Shiprocket cost is deducted from seller settlement.", fixed_customer: sellerShippingMode === "self" ? "Customer pays the fixed shipping amount; no logistics deduction is applied to seller settlement." : "Customer pays the fixed amount entered below; the difference from the live Shiprocket cost is adjusted in seller settlement.", estimated_seller: sellerShippingMode === "self" ? "Seller pays delivery; only platform and payment charges are deducted in settlement." : "Seller bears the configured shipping cost; it is not charged to the customer.", free_realtime: "Customer gets free shipping; the live Shiprocket cost is paid by the seller and deducted during settlement.", realtime_customer: "Customer pays the live Shiprocket rate at checkout." })[form.shippingMode || "free_included"]}</small></label>
