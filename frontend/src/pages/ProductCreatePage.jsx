...
export default function ProductCreatePage({ categories, taxCategories, sellerSettlement = {}, gstDetails = null, products = [], initialProduct, onSave, onBack, hideCostPrice = false, hideStatus = false, gstEnabled = true, sellerShippingMode = "shiprocket" }) {
  ...
  useEffect(() => {
    const next = toForm(initialProduct);
    if (next.shippingMode === "estimated_seller" || (sellerShippingMode !== "self" && next.shippingMode === "free_realtime")) {
      Object.assign(next, { shippingMode: "free_included", shippingIncludedInPrice: true, shippingPaidBy: "seller", shippingCharge: 0 });
    }
    setForm(next);
    setImageStatus("");
  }, [initialProduct, gstEnabled, sellerShippingMode]);
  ...
  const submitProduct = async (event) => {
    ...
    try {
      await onSave({
        ...
        codCharge: sellerShippingMode === "self" && form.codAvailable && form.codChargePaidBy === "customer" ? Number(form.codCharge || 0) : 0,
      ...
      <form className="panel productForm" onSubmit={submitProduct}>
        ...
        <details className="productFormSection" open>
          ...
          <label>
            ...
            {profitCalculation.result.requiredSellingPrice > 0 && <small>Calculator suggestion: {rupees(profitCalculation.result.requiredSellingPrice)}. Your entered selling price remains unchanged.</small>}
          </label>
          <label>
          ...
          <fieldset className="productPaymentOptions fullWidthField"><legend>Payment Options</legend><label className="toggleRow"><input type="checkbox" checked={form.prepaidAvailable !== false} onChange={(event) => setField("prepaidAvailable", event.target.checked)} /><span><strong>Prepaid</strong><small>Allow UPI, Card and Net Banking payments.</small></span></label><label className="toggleRow"><input type="checkbox" checked={Boolean(form.codAvailable)} onChange={(event) => setForm((current) => ({ ...current, codAvailable: event.target.checked, ...(!event.target.checked && { codCharge: 0, codChargePaidBy: "seller" }) }))} /><span><strong>Cash on Delivery (COD)</strong><small>{sellerShippingMode === "self" ? "Collect payment yourself on delivery." : "Subject to ShipRocket COD serviceability."}</small></span></label>{form.codAvailable && <><label><span>COD charge paid by</span><select value={form.codChargePaidBy || "seller"} onChange={(event) => { const paidBy = event.target.value; setForm((current) => ({ ...current, codChargePaidBy: paidBy, codCharge: paidBy === "seller" ? 0 : current.codCharge })); }}><option value="customer">Customer</option><option value="seller">Seller</option></select><small>{sellerShippingMode === "self" ? form.codChargePaidBy === "customer" ? "The COD amount is added to the customer invoice for each unit." : "The seller bears the COD charge; the customer is charged ₹0." : form.codChargePaidBy === "customer" ? "Added to the customer invoice." : "Deducted from the seller settlement."}</small></label>{sellerShippingMode === "self" && form.codChargePaidBy === "customer" && <label>COD amount per unit (₹)<input type="number" min="0" step="0.01" required value={form.codCharge ?? 0} onChange={event => setField("codCharge", event.target.value)} /></label>}</>}</fieldset>
          <label><span>Customer Shipping</span><select value={form.shippingMode || "free_included"} onChange={(event) => { const mode = event.target.value; const customerPays = ["fixed_customer", "realtime_customer"].includes(mode); setForm((current) => ({ ...current, shippingMode: mode, shippingIncludedInPrice: !customerPays, shippingPaidBy: customerPays ? "customer" : "seller", shippingCharge: mode === "fixed_customer" ? current.shippingCharge : 0, shippingCost: sellerShippingMode === "self" ? 0 : current.shippingCost })); }}><option value="free_included">1. Free Shipping</option><option value="fixed_customer">{sellerShippingMode === "self" ? "2. Fixed Shipping charges by Customer" : "2. Fixed Shipping by Customer"}</option>{sellerShippingMode !== "self" && <option value="realtime_customer">3. Real-time Shipping charged to Customer</option>}</select><small>{({ free_included: sellerShippingMode === "self" ? "Customer sees Free Shipping; seller handles delivery with no shipping deduction in settlement." : "Customer gets free shipping; the live Shiprocket cost is deducted from seller settlement.", fixed_customer: sellerShippingMode === "self" ? "Customer pays the fixed shipping amount; no logistics deduction is applied to seller settlement." : "Customer pays the fixed amount entered below; the difference from the live Shiprocket cost is adjusted in seller settlement.", free_realtime: "Customer gets free shipping; the live Shiprocket cost is paid by the seller and deducted during settlement.", realtime_customer: "Customer pays the live Shiprocket rate at checkout." })[form.shippingMode || "free_included"]}</small></label>
          ...
          {sellerShippingMode !== "self" && <label><span>{["estimated_seller", "free_realtime", "realtime_customer"].includes(form.shippingMode) ? "Estimated Shiprocket cost" : "Actual shipping cost"}</span><input type="number" min="0" step="0.01" required value={form.shippingCost} onChange={(event) => setField("shippingCost", event.target.value)} /><small>Operational shipping value, separate from the optional profit calculator. Shiprocket replaces it with the live rate at order time.</small></label>}
          ...
          <details className={`sellerProfitCalculator productFormSection ${gstEnabled ? "" : "noGst"}`}>
            ...
            <div className="formGrid compact">
              ...
              <label><span>Payment gateway fee (%)</span><input type="number" readOnly value={sellerSettlement.paymentGatewayFeeRate ?? 2} /><small>Admin-configured rate; settlement also deducts 18% GST on this fee.</small></label>
