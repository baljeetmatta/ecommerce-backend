...
export default function PartnerPortal({ onBack, settings = {}, announcementContent }) {
  const [partner, setPartner] = useState(partnerAuthStore.partner);
  const initialReferralId = referralFromHash();
  const [screen, setScreen] = useState(partner ? "dashboard" : initialReferralId ? "register" : partnerScreenFromRoute());
  ...
  const [paymentOtp, setPaymentOtp] = useState({ challengeId: "", code: "", status: "", sending: false });
  const [paymentBypassEnabled, setPaymentBypassEnabled] = useState(false);
  const [registration, setRegistration] = useState(() => newRegistration(initialReferralId));
  ...
  useEffect(() => {
    ...
    api.storefrontPaymentMethods()
      .then((methods) => {
        const onlineMethods = (methods || []).filter((method) => method.type !== "cod");
        setOnlinePaymentMethods(onlineMethods);
        setSelectedPaymentMethod(onlineMethods[0]?.code || "");
      })
  ...
  useEffect(() => {
    const syncRegistrationRoute = () => {
      ...
      if (onCompleteRoute && registrationResult) setScreen("registered");
        ...
        setScreen("login");
      } else if (!partner && window.location.hash.split("?")[0] === "#/partner/signup") setScreen("register");
      else if (!partner && ["#/partner", "#/partner/login"].includes(window.location.hash.split("?")[0])) setScreen("login");
  ...
  const register = (event) => {
    ...
    setMessage("");
    if (!onlinePaymentMethods.length) { setMessage("No online payment method is currently available. Please contact support."); return; }
    setSelectedPaymentMethod((current) => current || onlinePaymentMethods[0].code);
    setPaymentSelectorOpen(true);
    setPaymentOtp({ challengeId: "", code: "", status: "Sending an OTP to your email…", sending: true });
    api.requestPartnerPaymentOtp(registration)
      .then((result) => setPaymentOtp({ challengeId: result.challengeId, code: "", status: result.message, sending: false }))
      .catch((error) => setPaymentOtp({ challengeId: "", code: "", status: error.message, sending: false }));
  };
  const verifyOtpAndProceedWithPayment = async () => {
    if (!selectedPaymentMethod) { setMessage("Please select a payment method."); return; }
    if (!paymentOtp.challengeId || paymentOtp.code.length !== 6) { setPaymentOtp((current) => ({ ...current, status: "Enter the 6-digit OTP sent to your email." })); return; }
    setBusy(true);
    setPaymentOtp((current) => ({ ...current, status: "Verifying email OTP…" }));
    try {
      const verified = await api.verifyPartnerPaymentOtp({ challengeId: paymentOtp.challengeId, code: paymentOtp.code });
      setPaymentOtp((current) => ({ ...current, status: verified.message }));
      const order = await api.createPartnerRegistrationOrder({ ...registration, paymentMethodCode: selectedPaymentMethod, otpChallengeId: verified.challengeId, otpVerificationToken: verified.verificationToken, returnUrl: window.location.href });
      setPaymentSelectorOpen(false);
      const payment = await runPartnerCheckout(order, { name: registration.name, email: registration.email, contact: registration.mobile }, { kind: "partner-registration", registration });
      const result = await api.partnerRegister({ ...registration, payment });
      showRegistrationComplete(result);
      setMessage(result.message);
    } catch (error) {
      setPaymentOtp((current) => ({ ...current, status: error.message }));
    } finally {
      setBusy(false);
    }
  };
  ...
  const openBlankRegistration = () => {
    clearPayuReturn();
    setRegistration(newRegistration(initialReferralId));
  ...
  if (!partner && screen === "registered" && registrationResult) return <PartnerRegistrationSuccess result={registrationResult} onContinue={() => { setLogin({ registrationNumber: registrationResult.registrationNumber, password: "" }); setMessage(""); window.history.pushState(null, "", "#/partner/login"); setScreen("login"); }} />;
  if (!partner && screen === "verify-otp") return <PortalAuthCard portal="Partner" heading="Verify your email" subtitle="Enter the 6-digit code sent to your email to complete registration." dividerText="Email verification"><form className="authForm" onSubmit={verifyRegistrationOtp}>{message && <div className="notice">{message}</div>}<label><span>Email verification code</span><OtpInput inputMode="numeric" pattern="\d{6}" maxLength="6" autoComplete="one-time-code" required value={registrationOtp.code} onChange={(event) => setRegistrationOtp({ ...registrationOtp, code: event.target.value.replace(/\D/g, "").slice(0, 6) })} /></label><button className="primaryButton authButton" disabled={busy || registrationOtp.code.length !== 6}>{busy ? "Verifying…" : "Verify and create account"}</button><button className="linkButton" type="button" onClick={() => { setMessage(""); setScreen("register"); }}>Back to registration</button></form></PortalAuthCard>;

  if (!partner && screen === "login") return <PartnerLoginScreen settings={settings} onBack={onBack} message={message} login={login} setLogin={setLogin} busy={busy} onSubmit={signIn} onForgot={() => setScreen("forgot")} onSignup={openBlankRegistration} />;
  ...
  const navigation = [["dashboard", "Dashboard", LayoutGrid], ["referrals", "Referrals", UserPlus], ["profile", "Profile Update", UserRound], ["password", "Change Password", KeyRound], ["kyc", "KYC", FileCheck2], ["bank", "Bank Details", Building2], ["payouts", "Payouts", BadgeIndianRupee], ["withdrawal", "Withdrawal", Mail], ["support", "Support Tickets", Headphones]];
  ...
  return <div className={`partnerShell premiumPartnerWorkspace ${mobileNavOpen ? "navOpen" : ""}`}><MobileBottomNav label="Partner" active={screen} items={[
    { id: "dashboard", label: "Home", icon: "home" },
    { id: "referrals", label: "Referrals", icon: "referrals" },
    { id: "payouts", label: "Earnings", icon: "payouts" },
    { id: "withdrawal", label: "Withdraw", icon: "payouts", current: screen.startsWith("withdrawal") },
    { id: "more", label: "More", icon: "more", current: mobileNavOpen || !["dashboard", "referrals", "payouts", "withdrawal", "withdrawal-new"].includes(screen) }
  ]} onSelect={(id) => { if (id === "more") setMobileNavOpen(true); else { setScreen(id); setMessage(""); setMobileNavOpen(false); } }} /><button className="partnerNavBackdrop" aria-label="Close navigation" onClick={() => setMobileNavOpen(false)} /><aside className="partnerNav"><div className="brand"><div className="partnerBrandIcon"><ShoppingCart size={25} /></div><div><strong>HR BASKET</strong><span>Partner Portal</span></div><button className="partnerNavClose" onClick={() => setMobileNavOpen(false)} aria-label="Close menu"><X /></button></div><nav>{navigation.map(([id, label, Icon]) => <button key={id} className={screen === id || (id === "withdrawal" && screen === "withdrawal-new") ? "active" : ""} onClick={() => { setScreen(id); setMessage(""); setMobileNavOpen(false); }}><span><Icon size={19} /></span>{label}</button>)}<button onClick={logout}><span><LogOut size={19} /></span>Logout</button></nav><div className="partnerSidebarPromo"><Gift /><Sparkles className="promoSparkle" /><strong>Earn More<br />Grow Together</strong><button onClick={() => { setScreen("referrals"); setMobileNavOpen(false); }}>Refer &amp; Earn More <span>→</span></button></div></aside><main className="partnerContent"><header><button className="partnerMenuButton" onClick={() => setMobileNavOpen(true)} aria-label="Open menu"><Menu /></button><div className="partnerPageHeading"><h1>{announcementContent ? "Announcements" : screen === "dashboard" ? <>{greeting === "Good morning" ? "👋" : "✨"} {greeting}, <span>{partner.name}!</span></> : pageTitle}</h1><p>{screen === "dashboard" ? "Here’s an overview of your partner account." : `Partner ID: ${partner.registrationNumber}`}</p></div><div className="partnerHeaderActions"><div className="partnerHeaderAvatar" aria-label={`${partner.name} profile`}>{partner.profileImage ? <img src={partner.profileImage} alt="" /> : <span>{partner.name?.trim()?.[0]?.toUpperCase() || "P"}</span>}</div><div className="partnerHeaderIdentity"><strong>{partner.name}</strong><span>ID: {partner.registrationNumber}</span></div></div></header>{message && <div className="notice">{message}</div>}{announcementContent || <>{screen === "dashboard" && <DashboardAnnouncements announcements={settings.announcements} audience="partner" show="text" />}{screen === "dashboard" && <Dashboard data={data.dashboard} partner={partner} packages={packages} onChangePackage={changePackage} />}{screen === "dashboard" && <DashboardAnnouncements announcements={settings.announcements} audience="partner" show="media" />}{screen === "referrals" && <Referrals partner={partner} data={data.dashboard} setMessage={setMessage} />}{screen === "profile" && <ProfileSettings role="partner"><Profile partner={partner} save={(payload) => submit(async () => { await api.partnerUpdateProfile(payload); await refresh(); setMessage("Profile updated."); })} /></ProfileSettings>}{screen === "password" && <ChangePassword save={(payload) => submit(async () => { const result = await api.partnerChangePassword(payload); setMessage(result.message); })} />}{screen === "kyc" && <Kyc partner={partner} save={async (type, payload) => { const updated = await api.partnerUploadKyc(type, payload); partnerAuthStore.partner = updated; setPartner(updated); return updated; }} fileData={fileData} />}{screen === "bank" && <Bank partner={partner} save={(payload) => submit(async () => { await api.partnerUpdateBank(payload); await refresh(); setMessage("Bank details updated."); })} />}{screen === "payouts" && <Ledger rows={data.payouts} />}{screen === "withdrawal" && <Withdrawals rows={data.withdrawals} balance={data.dashboard.walletBalance} minimumAmount={data.dashboard.minimumWithdrawalAmount} onNew={() => { setMessage(""); setScreen("withdrawal-new"); }} />}{screen === "support" && <SupportTickets accountType="Partner" />}{screen === "withdrawal-new" && <WithdrawalRequest balance={data.dashboard.walletBalance} minimumAmount={data.dashboard.minimumWithdrawalAmount} email={partner.email} onBack={() => setScreen("withdrawal")} onComplete={async () => { await refresh(); setMessage("Withdrawal request submitted successfully for admin review."); setScreen("withdrawal"); }} />}</>}</main></div>;
}
...
const Dashboard = ({ data, partner, packages, onChangePackage }) => {
  ...
  return <div className="premiumPartnerDashboard">
    ...
    <section className="partnerDashboardWelcome">
      <button className="partnerDashboardAvatar" type="button" disabled={!partner.profileImage} onClick={() => setProfilePreviewOpen(true)} aria-label={partner.profileImage ? "Open profile image" : `${partner.name} profile`}>{partner.profileImage ? <img src={partner.profileImage} alt={`${partner.name} profile`} /> : <span>{partner.name?.trim()?.[0]?.toUpperCase() || "P"}</span>}</button>
    ...
    <div className="partnerDashboardColumns">
      ...
      <div className="summaryGrid">{stats.map(([tone, icon, label, value]) => { const Icon = typeof icon === "string" ? null : icon; return <article className={tone} key={label}><div className="statIcon">{Icon ? <Icon /> : <img src={icon} alt="" />}</div><div><span>{label}</span><strong>{value}</strong></div>{Icon ? <Icon className="statWatermark" /> : <img className="statWatermark" src={icon} alt="" />}</article>; })}</div>
    </div>
    ...
    {profilePreviewOpen && <ImagePreview src={partner.profileImage} alt={`${partner.name} profile`} onClose={() => setProfilePreviewOpen(false)} />}
  </div>;
};
function Referrals({ partner, data, setMessage }) { const referralUrl = `${window.location.origin}${window.location.pathname}#/partner/register?ref=${partner.registrationNumber}`; const copy = async () => { try { await navigator.clipboard.writeText(referralUrl); setMessage("Referral URL copied."); } catch (_error) { setMessage("Copy failed. Select and copy the URL manually."); } }; return <><div className="panel referralPanel"><h3>Your referral URL</h3><p>Share this URL. New partners who use it will be linked to your partner ID <strong>{partner.registrationNumber}</strong>.</p><div className="referralUrl"><input readOnly value={referralUrl} onFocus={(event) => event.target.select()} /><button className="primaryButton" type="button" onClick={copy}><Copy size={17} />Copy</button><a className="whatsAppShareButton" href={`https://wa.me/?text=${encodeURIComponent(`Join HRS Basket as a partner using my referral link: ${referralUrl}`)}`} target="_blank" rel="noreferrer"><WhatsAppIcon /> WhatsApp</a></div></div><div className="summaryGrid referralSummary"><article><span>Direct referrals</span><strong>{data.referralCount || 0}</strong></article></div><div className="panel tableWrap"><table><thead><tr><th>Joined</th><th>Partner ID</th><th>Name</th><th>Email</th><th>Status</th></tr></thead><tbody>{(data.recentReferrals || []).map((item) => <tr key={item._id}><td>{new Date(item.createdAt).toLocaleDateString("en-IN")}</td><td>{item.registrationNumber}</td><td>{item.name}</td><td>{item.email}</td><td>{item.status}</td></tr>)}{!data.recentReferrals?.length && <tr><td colSpan="5">No referrals yet.</td></tr>}</tbody></table></div></>; }
...
function ImagePreview({ src, alt, onClose }) { return <div className="profileImageOverlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section role="dialog" aria-modal="true" aria-label="Profile image preview"><button type="button" onClick={onClose} aria-label="Close image preview"><X /></button><img src={src} alt={alt} /></section></div>; }
function Profile({ partner, save }) {
  const [form, setForm] = useState({ address: partner.address, profileImage: partner.profileImage || "" });
  const [previewOpen, setPreviewOpen] = useState(false);
  const [uploading, setUploading] = useState(false);
  return <><ProfileSummary account={partner} role="Partner" /><form className="panel partnerProfileCard" onSubmit={(e) => { e.preventDefault(); save(form); }}><aside className="partnerProfilePreview">{form.profileImage ? <button className="partnerProfileImageButton" type="button" onClick={() => setPreviewOpen(true)} aria-label="Open profile image"><img src={form.profileImage} alt="Profile preview" /></button> : <div className="partnerProfileFallback">{partner.name?.[0]}</div>}<strong>{partner.name}</strong><span>{partner.registrationNumber}</span><label className="secondaryButton">{uploading ? "Compressing…" : "Change photo"}<input hidden type="file" accept="image/*" disabled={uploading} onChange={async (e) => { const file = e.target.files?.[0]; if (!file) return; setUploading(true); try { const uploaded = await api.uploadImage(file, "partner-profile"); setForm((current) => ({ ...current, profileImage: uploaded.url })); } finally { setUploading(false); e.target.value = ""; } }} /></label><small>Compressed WebP, maximum 400×400 px</small></aside><div className="formGrid twoColumn"><label>Name (non-editable)<input disabled value={partner.name} /></label><label>Email (non-editable)<input disabled value={partner.email} /></label><label className="full">Address<input value={form.address.line} onChange={(e) => setForm({ ...form, address: { ...form.address, line: e.target.value } })} /></label><label>City<input value={form.address.city} onChange={(e) => setForm({ ...form, address: { ...form.address, city: e.target.value } })} /></label><label>State<input value={form.address.state} onChange={(e) => setForm({ ...form, address: { ...form.address, state: e.target.value } })} /></label><label>Postal code<input value={form.address.postalCode || ""} onChange={(e) => setForm({ ...form, address: { ...form.address, postalCode: e.target.value } })} /></label><button className="primaryButton">Save profile</button></div></form>{previewOpen && <ImagePreview src={form.profileImage} alt={`${partner.name} profile`} onClose={() => setPreviewOpen(false)} />}</>;
}
