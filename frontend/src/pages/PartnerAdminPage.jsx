...
export default function PartnerAdminPage({ activeTab = "partners", onTabChange, onViewDetails, detailOnly = false, detailId, onBack, onDelete }) {
  ...
  return <section>
    ...
    {activeTab === "withdrawals" && <div className="panel tableWrap"><table><thead><tr><th>Partner</th><th>Bank</th><th>Amount</th><th>Status</th><th>Payment details</th><th>Action</th></tr></thead><tbody>{data.withdrawals.map((item) => <tr key={item._id}><td>{item.partner?.name}<br />{item.partner?.email}</td><td>{item.bankSnapshot?.bankName}<br />{item.bankSnapshot?.accountNumber} · {item.bankSnapshot?.ifsc}</td><td>{money(item.amount)}</td><td>{item.status}</td><td>{item.paidAt ? new Date(item.paidAt).toLocaleDateString("en-IN") : "—"}<br />{item.adminNote || ""}</td><td>{item.status === "pending" && <><button onClick={() => act(() => api.processWithdrawal(item._id, { status: "approved" }))}>Approve</button><button onClick={() => act(() => api.processWithdrawal(item._id, { status: "rejected", adminNote: "Rejected by administrator" }))}>Reject & refund</button></>}{item.status === "approved" && <button onClick={() => setPaymentTarget(item)}>Mark paid</button>}</td></tr>)}</tbody></table></div>}
    {paymentTarget && <PaidWithdrawalDialog title="Mark partner withdrawal as paid" onCancel={() => setPaymentTarget(null)} onConfirm={async (payload) => { await act(() => api.processWithdrawal(paymentTarget._id, { status: "paid", ...payload })); setPaymentTarget(null); }} />}
...
function PaidWithdrawalDialog({ title, onCancel, onConfirm }) { const [adminNote, setAdminNote] = useState(""); const [paidAt, setPaidAt] = useState(new Date().toISOString().slice(0, 10)); const [busy, setBusy] = useState(false); return <div className="partnerDetailsOverlay"><form className="partnerDetailsDialog compactDialog" role="dialog" aria-modal="true" onSubmit={async (event) => { event.preventDefault(); setBusy(true); try { await onConfirm({ adminNote: adminNote.trim(), paidAt }); } finally { setBusy(false); } }}><h2>{title}</h2><label>Payment date<input type="date" required value={paidAt} onChange={(event) => setPaidAt(event.target.value)} /></label><label>Payment comments<textarea autoFocus required rows="4" placeholder="Transaction reference or payment note" value={adminNote} onChange={(event) => setAdminNote(event.target.value)} /></label><div className="kycActions"><button className="secondaryButton" type="button" disabled={busy} onClick={onCancel}>Cancel</button><button className="primaryButton" disabled={busy || !adminNote.trim()}>{busy ? "Saving…" : "Confirm paid"}</button></div></form></div>; }
