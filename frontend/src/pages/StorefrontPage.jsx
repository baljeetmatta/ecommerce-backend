...
function ReturnRequestDialog({ target, onCancel, onComplete }) {
  ...
  return <div className="modalOverlay" role="dialog" aria-modal="true"><form className="sellerStatusModal" onSubmit={async (event) => { event.preventDefault(); setBusy(true); setError(""); try { const updated = await api.requestCustomerItemReturn(target.order._id, target.item.product?._id || target.item.product, { reason, comments }); onComplete(updated); } catch (requestError) { setError(requestError.message); } finally { setBusy(false); } }}><div className="panelHeader"><div><span className="eyebrow">Product return</span><h2>Return {target.item.name}</h2></div><button className="inlineButton" type="button" disabled={busy} onClick={onCancel}>Close</button></div><label>Return reason<select required value={reason} onChange={(event) => setReason(event.target.value)}><option value="">Select reason</option><option>Damaged product</option><option>Wrong item received</option><option>Product not as described</option><option>Quality issue</option><option>Other</option></select></label><label>Additional comments<textarea rows="4" value={comments} onChange={(event) => setComments(event.target.value)} placeholder="Describe the issue for the seller and administrator" /></label>{error && <p className="errorText">{error}</p>}<button className="heroPrimary" disabled={busy || !reason}>{busy ? "Submitting…" : "Submit return request"}</button></form></div>;
}
