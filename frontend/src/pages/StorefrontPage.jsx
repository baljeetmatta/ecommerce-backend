...
const resolveBackendMediaUrl = (value) => {
  ...
  const apiUrl = import.meta.env.VITE_API_URL || "https://ebackend.hrsbasket.com/api";
  const backendOrigin = new URL(apiUrl, window.location.origin).origin;
  const uploadPath = source.startsWith("/api/uploads/") ? source.slice(4) : source.startsWith("api/uploads/") ? `/${source.slice(4)}` : source;
  return `${backendOrigin}/${uploadPath.replace(/^\/+/, "")}`;
};

const productReelUrl = (product) => resolveBackendMediaUrl(
  product.videoUrl ||
...
export default function StorefrontPage({ products, featuredProducts, categories, banner, heroItems = [], contentSections = [], productBanners = [], productBannerColumns = 2, firstOrderDiscount = null, blogPosts = [], settings = {}, paymentMethods = [], shippingRules = [], storefrontLoading = false, storefrontError = "", onReloadStorefront, onAdminLogin }) {
  ...
  const displayCodCharge = checkout.paymentType === "cod" ? Number(shiprocketQuote?.codChargedToCustomer || 0) + cart.reduce((sum, item) => sum + (item.product.seller?.shippingMode === "self" && item.product.codChargePaidBy === "customer" ? Number(item.product.codCharge || 0) * item.quantity : 0), 0) : 0;
  const needsCodQuote = (item) => item.product.seller?.shippingMode !== "self" && checkout.paymentType === "cod" && (item.product.codChargePaidBy === "customer" || item.product.seller?.shippingMode === "shiprocket");
  const hasRealtimeCustomerShipping = cart.some((item) => (item.product.seller?.shippingMode !== "self" && item.product.shippingMode === "realtime_customer") || needsCodQuote(item));
  ...
  useEffect(() => {
    const liveItems = cart.filter((item) => (item.product.seller?.shippingMode !== "self" && item.product.shippingMode === "realtime_customer") || needsCodQuote(item));
    ...
    <div className="storefront" style={{ "--mobile-product-grid-size": settings.mobileProductGridSize || 2 }}>
      ...
      <header className="shopHeader">
        ...
        <nav className={mobileMenuOpen ? "shopNav open" : "shopNav"} aria-label="Primary navigation">
          ...
          <button className={isReelsRoute ? "active" : ""} type="button" onClick={() => { setMobileMenuOpen(false); navigate("#/reels"); }}>Reels</button>
...
function SellerReelsGallery({ products, seller, loading, error, onRetry, onOpen, onBack }) {
  if (loading) return <section className="shopSection emptyRoute"><h2>Loading seller reels…</h2></section>;
  if (error) return <section className="shopSection emptyRoute"><h2>Could not load seller reels</h2><p>{error}</p><button className="heroPrimary" type="button" onClick={onRetry}>Try again</button></section>;

  return <section className="sellerReelsPage">
    <header>
      <button className="shopLinkButton" type="button" onClick={onBack}>Back to all reels</button>
      ...
      <div className="flatProductTop">
        ...
        <div className="flatProductInfo">
          ...
          <div className="flatDetailMeta">
            ...
            {product.seller && product.codAvailable === true && <span><WalletCards size={16} /> {product.seller.shippingMode === "self" ? "COD available · seller delivery" : "COD available subject to pincode"}</span>}
...
function FlatReviews({ product, customer }) {
  ...
  const submit = async (event) => { event.preventDefault(); setStatus(""); try { const result = await api.createProductReview(product._id, form); setStatus(result.message || "Your review is awaiting approval."); setFormOpen(false); setForm({ rating: 5, sellerRating: 5, comment: "" }); } catch (error) { setStatus(error.message); } };
    ...
    <section className="flatReviews" id="product-reviews">
      ...
      {reviews.map((review) => (
        <article key={review._id}>
          <div className="reviewAvatar">{review.profileImage ? <img src={review.profileImage} alt="" /> : review.name?.charAt(0)?.toUpperCase()}</div>
          <div>
            <header>
...
function CustomerOrderItemRow({ order, item, index, onReturn, onReview, onViewDetails }) {
  ...
  const openDetails = (event) => { event.stopPropagation(); onViewDetails({ order, item }); };
  return <div><button className="customerOrderProduct" type="button" onClick={openDetails} aria-label={`View details for ${item.name}`}><img src={item.product ? productImage(item.product) : "/images/e-commerce/home/product4.png"} onError={(event) => useProductImageFallback(event, item.product || item)} alt={item.name} /></button><div><button className="customerOrderProductName" type="button" onClick={openDetails}><strong>{item.name}</strong></button><span>{item.sku} · Qty {item.quantity}</span><small className={`itemOrderStatus ${String(item.sellerStatus || order.status).toLowerCase().replaceAll(" ", "-")}`}>Item status: {item.sellerStatus || order.status}</small>{item.returnApplicable && item.returnDays > 0 && <small>Return window: until {returnDeadline.toLocaleDateString("en-IN")}</small>}{order.shipping?.awbCode && <small>Tracking ID: {order.shipping.awbCode}</small>}{item.returnRequest?.status && <small>Return: {item.returnRequest.status}{item.returnRequest.reviewNote ? ` · ${item.returnRequest.reviewNote}` : ""}</small>}{item.returnRequest?.reason && <p>Return reason: {item.returnRequest.reason}</p>}<ReturnEvidence evidence={item.returnRequest?.evidence} />{canReturn && <button className="shopLinkButton" type="button" onClick={() => onReturn({ order, item })}>Return item by {returnDeadline.toLocaleDateString("en-IN")}</button>}{canReview && <button className="shopLinkButton orderReviewButton" type="button" onClick={() => onReview({ order, item })}><Star size={15} /> Rate product &amp; store</button>}{!item.returnApplicable && <small>Not returnable</small>}</div><strong>{money(item.price * item.quantity)}</strong></div>;
}

function OrderedProductDetailsDialog({ target, onClose }) {
  const { order, item } = target;
  const product = item.product && typeof item.product === "object" ? item.product : {};
  const description = product.shortDescription || product.detailedDescription || product.description;
  const variants = item.variantAttributes instanceof Map ? Object.fromEntries(item.variantAttributes) : item.variantAttributes;
  return <div className="modalOverlay orderedProductOverlay" role="dialog" aria-modal="true" aria-labelledby="ordered-product-title" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="orderedProductDialog"><button className="modalClose" type="button" onClick={onClose} aria-label="Close product details"><X size={20} /></button><div className="orderedProductImage"><img src={productImage(product._id ? product : item, "detail")} onError={(event) => useProductImageFallback(event, product._id ? product : item)} alt={item.name} /></div><div className="orderedProductInfo"><span className="eyebrow">Ordered product</span><h2 id="ordered-product-title">{item.name}</h2>{product.manufacturerBrand && <p className="orderedProductBrand">{product.manufacturerBrand}</p>}{description && <p className="orderedProductDescription">{description}</p>}<dl><div><dt>Order number</dt><dd>{order.orderNumber}</dd></div><div><dt>Order date</dt><dd>{new Date(order.createdAt).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" })}</dd></div><div><dt>SKU</dt><dd>{item.sku}</dd></div><div><dt>Quantity</dt><dd>{item.quantity}</dd></div><div><dt>Price</dt><dd>{money(item.price)}</dd></div><div><dt>Item total</dt><dd>{money(item.price * item.quantity)}</dd></div><div><dt>Status</dt><dd>{item.sellerStatus || order.status}</dd></div><div><dt>Payment</dt><dd>{order.payment?.methodName || order.paymentStatus}</dd></div>{variants && Object.entries(variants).map(([name, value]) => <div key={name}><dt>{name}</dt><dd>{value}</dd></div>)}{product.warranty && <div><dt>Warranty</dt><dd>{product.warranty}</dd></div>}{product.countryOfOrigin && <div><dt>Country of origin</dt><dd>{product.countryOfOrigin}</dd></div>}<div><dt>Return policy</dt><dd>{item.returnApplicable && item.returnDays > 0 ? `${item.returnDays} days` : "Not returnable"}</dd></div></dl>{product._id && <a className="heroSecondary" href={`#/product/${encodeURIComponent(product._id)}`} onClick={onClose}>View product page</a>}</div></section></div>;
}
...
function CustomerDashboard({ customer, setCustomer, onLogout, onClose, pageMode = false }) {
  ...
  const [profile, setProfile] = useState({ name: customer.name || "", email: customer.email || "", phone: customer.phone || "", profileImage: customer.profileImage || "", gender: customer.gender || "prefer_not_to_say" });
  ...
  const [productDetailsTarget, setProductDetailsTarget] = useState(null);
  ...
  useEffect(() => {
    Promise.all([api.customerAccount(), api.customerOrders(1)])
      .then(([account, orderData]) => {
        ...
        setCustomer(next);
        setProfile({ name: next.name || "", email: next.email || "", phone: next.phone || "", profileImage: next.profileImage || "", gender: next.gender || "prefer_not_to_say" });
  ...
  return <div className={pageMode ? "shopSection customerAccountPage" : "modalOverlay customerDashboardOverlay"} role={pageMode ? undefined : "dialog"} aria-modal={pageMode ? undefined : "true"} aria-label="Customer dashboard">
    <section className="customerDashboard">
      ...
      <aside className="customerDashboardNav">
        <div className="customerIdentity"><div>{customer.profileImage ? <img src={customer.profileImage} alt="" /> : customer.name?.charAt(0)?.toUpperCase()}</div><strong>{customer.name}</strong><span>{customer.email}</span></div>
        {[['orders','Order History'],['profile','Profile Update'],['addresses','Manage Addresses'],['support','Support Tickets']].map(([id,label]) => <button key={id} className={tab === id ? "active" : ""} type="button" onClick={() => { setTab(id); setStatus(""); }}>{label}</button>)}
      ...
      <div className="customerDashboardContent">
        ...
        {!loading && tab === "orders" && <><span className="eyebrow">Your purchases</span><h2>Order History</h2>{!orders.length && <div className="accountEmpty"><PackageCheck size={34} /><h3>No orders yet</h3><p>Your completed orders will appear here.</p></div>}<div className="customerOrderList">{orders.map((order) => { const displayStatus = order.items?.[0]?.sellerStatus || order.status; return <article key={order._id}><header><div><strong>{order.orderNumber}</strong><span>{new Date(order.createdAt).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" })}</span></div><span className={`orderStatus ${String(displayStatus).toLowerCase().replaceAll(" ", "-")}`}>{displayStatus}</span></header><div className="customerOrderItems">{order.items.map((item, index) => <CustomerOrderItemRow key={`${item.product?._id || item.sku}-${index}`} order={order} item={item} index={index} onReturn={setReturnTarget} onReview={setRatingTarget} onViewDetails={setProductDetailsTarget} />)}</div><footer><span>Payment: {order.payment?.methodName || order.paymentStatus}</span><button className="orderTrackIcon" type="button" title="Track order" aria-label={`Track ${order.orderNumber}`} onClick={() => setTrackingOrder(order)}><MapPin size={17} /></button><strong>Total {money(order.grandTotal)}</strong></footer></article>; })}</div>{orderPagination.pages > 1 && <nav className="customerOrderPagination" aria-label="Order history pages"><button type="button" disabled={orderPagination.page <= 1} onClick={() => loadOrders(orderPagination.page - 1)}>Previous</button><span>Page {orderPagination.page} of {orderPagination.pages}</span><button type="button" disabled={orderPagination.page >= orderPagination.pages} onClick={() => loadOrders(orderPagination.page + 1)}>Next</button></nav>}</>}
        {!loading && tab === "profile" && <><span className="eyebrow">Personal details</span><h2>Profile Update</h2><form className="accountForm" onSubmit={saveProfile}><label className="customerPhotoField"><span>Profile photo</span>{profile.profileImage ? <img src={profile.profileImage} alt="Your profile" /> : <span className="customerPhotoPlaceholder">{profile.name?.charAt(0)?.toUpperCase() || "?"}</span>}<input type="file" accept="image/*" onChange={async (event) => { const file = event.target.files?.[0]; if (!file) return; setStatus("Uploading profile photo…"); try { const uploaded = await api.uploadImage(file, "customer-profile"); setProfile((current) => ({ ...current, profileImage: uploaded.url })); setStatus("Profile photo uploaded. Save your profile to apply it."); } catch (error) { setStatus(error.message); } }} /></label><label>Name<input required value={profile.name} onChange={(event) => setProfile({ ...profile, name: event.target.value })} /></label><label>Email<input value={profile.email} disabled /></label><label>Phone<input value={profile.phone} onChange={(event) => setProfile({ ...profile, phone: event.target.value })} /></label><label>Gender<select value={profile.gender} onChange={(event) => setProfile({ ...profile, gender: event.target.value })}><option value="female">Female</option><option value="male">Male</option><option value="other">Other</option><option value="prefer_not_to_say">Prefer not to say</option></select></label><button className="heroPrimary">Save Profile</button></form></>}
        {!loading && tab === "support" && <SupportTickets accountType="Customer" orders={orders} />}{!loading && tab === "addresses" && <><div className="accountTitleRow"><div><span className="eyebrow">Saved delivery details</span><h2>Manage Addresses</h2></div><button className="heroSecondary" type="button" onClick={addAddress}><Plus size={16} /> Add Address</button></div><div className="addressEditorList">{addresses.map((address, index) => <article key={address._id || index}><label className="toggleRow"><input type="radio" name="defaultAddress" checked={Boolean(address.isDefault)} onChange={() => setDefaultAddress(index)} /><span>Use as default shopping address</span></label>{['label','line1','city','state','postalCode','country'].map((field) => <label key={field}>{field === 'line1' ? 'Address line' : field === 'postalCode' ? 'Postal code' : field}<input required value={address[field] || ""} onChange={(event) => updateAddress(index, field, event.target.value)} /></label>)}<button className="shopLinkButton" type="button" onClick={() => setAddresses((current) => current.filter((_item, itemIndex) => itemIndex !== index))}>Remove</button></article>)}</div>{addresses.length > 0 && <button className="heroPrimary" type="button" onClick={saveAddresses}>Save Addresses</button>}{!addresses.length && <div className="accountEmpty"><h3>No saved addresses</h3><p>Add an address for faster checkout.</p></div>}</>}{returnTarget && <ReturnRequestDialog target={returnTarget} onCancel={() => setReturnTarget(null)} onComplete={(updated) => { setOrders((current) => current.map((order) => order._id === updated._id ? updated : order)); setReturnTarget(null); setStatus("Return request submitted successfully. The seller and administrator have been notified."); }} />}{trackingOrder && <OrderTrackingDialog order={trackingOrder} onClose={() => setTrackingOrder(null)} />}{ratingTarget && <OrderReviewDialog target={ratingTarget} onClose={() => setRatingTarget(null)} />}{productDetailsTarget && <OrderedProductDetailsDialog target={productDetailsTarget} onClose={() => setProductDetailsTarget(null)} />}
      </div>
    </section>
  </div>;
}

function ReturnRequestDialog({ target, onCancel, onComplete }) {
  const [evidence, setEvidence] = useState([]); const [uploading, setUploading] = useState(false); const [category, setCategory] = useState("Unboxing Photo");
  const [reason, setReason] = useState(""); const [comments, setComments] = useState(""); const [busy, setBusy] = useState(false); const [error, setError] = useState("");
  return <div className="modalOverlay" role="dialog" aria-modal="true"><form className="sellerStatusModal" onSubmit={async (event) => { event.preventDefault(); setBusy(true); setError(""); try { const updated = await api.requestCustomerItemReturn(target.order._id, target.item.product?._id || target.item.product, { reason, comments, evidence }); onComplete(updated); } catch (requestError) { setError(requestError.message); } finally { setBusy(false); } }}><div className="panelHeader"><div><span className="eyebrow">Product return</span><h2>Return {target.item.name}</h2></div><button className="inlineButton" type="button" disabled={busy} onClick={onCancel}>Close</button></div><fieldset><legend>Return reason (select one)</legend>{["Damaged product", "Wrong item received", "Product not as described", "Quality issue", "Other"].map(option => <label className="toggleRow" key={option}><input type="radio" name="returnReason" required value={option} checked={reason === option} onChange={() => setReason(option)} />{option}</label>)}</fieldset><label>Evidence category<select value={category} onChange={e=>setCategory(e.target.value)}>{["Unboxing Photo", "Unboxing Video", "Product Damage Video", "Product Label Photo"].map(label=><option key={label}>{label}</option>)}</select></label><label>Upload return evidence (required)<input type="file" disabled={uploading || busy} accept={["Unboxing Photo", "Product Label Photo"].includes(category) ? "image/*" : "video/*"} onChange={async e=>{const file=e.target.files?.[0]; if(!file)return; setUploading(true); setError(""); try { const result=["Unboxing Photo", "Product Label Photo"].includes(category) ? await api.uploadImage(file, "return-evidence") : await api.uploadVideo(file); setEvidence(current=>[...current,{category,url:result.url}]); } catch(error){setError(error.message)} finally{setUploading(false); e.target.value="";} }}/></label>{uploading && <p>Uploading evidence…</p>}{evidence.map((entry,index)=><p key={entry.url}><a href={entry.url} target="_blank" rel="noreferrer">{entry.category}</a> <button type="button" onClick={()=>setEvidence(current=>current.filter((_,i)=>i!==index))}>Remove</button></p>)}<label>Additional comments<textarea rows="4" value={comments} onChange={(event) => setComments(event.target.value)} placeholder="Describe the issue for the seller and administrator" /></label>{error && <p className="errorText">{error}</p>}<button className="heroPrimary" disabled={busy || uploading || !reason || !evidence.length}>{busy ? "Submitting…" : "Submit return request"}</button></form></div>;
}
