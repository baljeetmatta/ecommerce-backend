...
function SellerDashboard({ data }) {
  ...
  return <div className="sellerDashboardV3">
    ...
    <section className={`partnerOnboarding sellerAccountSetup ${setupApproved ? "approved completedSetup" : "hasPending"} ${setupCollapsed ? "collapsed" : ""}`}><div className="setupMain"><div className="setupHeading"><div><h3>{setupApproved ? "Account setup complete" : setupCollapsed ? `Account setup pending at step ${setupPendingIndex + 1}` : "Account Setup"}</h3>{!setupApproved && !setupCollapsed && <p>Complete each step to activate all seller benefits.</p>}</div></div>{!setupApproved && !setupCollapsed && <div className="onboardingSteps">{setupSteps.map(([label, complete], index) => <button type="button" key={label} className={`onboardingStep ${complete ? "completed" : "pending"}`} onClick={() => onNavigate(label === "Bank details" ? "bank" : label === "KYC verification" ? "kyc" : "profile")}><span>{index + 1}</span><div><strong>{label}</strong><small>{complete ? "Completed" : "Pending"}</small></div></button>)}</div>}</div><div className="setupActions"><div className="setupShield">{setupApproved ? <ShieldCheck /> : !setupCollapsed && <><ShieldAlert /><b>{Math.round((setupSteps.filter(([, complete]) => complete).length / setupSteps.length) * 100)}%</b></>}</div>{!setupApproved && <button type="button" className="setupMinimize" onClick={() => setSetupCollapsed((value) => !value)} aria-label={setupCollapsed ? "Expand account setup" : "Minimize account setup"}>{setupCollapsed ? <ChevronDown /> : <Minus />}</button>}</div></section>
...
function SellerOrders({ orders, update, returnUpdate, action, shippingMode }) {
  const statuses = shippingMode === "shiprocket" ? ["Pending", "Accepted", "Processing", "Packed", "Ready to Dispatch", "Shipped", "Delivered", "Cancelled"] : ["Pending", "Accepted", "Processing", "Packed", "Ready to Dispatch", "Shipped", "Delivered", "Cancelled"];
  ...
  return <section className="sellerOrdersPage">
    ...
    {selectedOrder && <SellerOrderDetails order={selectedOrder} tab={detailTab} setTab={setDetailTab} onClose={() => setSelectedOrder(null)} />}
