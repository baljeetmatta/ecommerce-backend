...
function SellerDashboard({ data }) {
  ...
  return <div className="sellerDashboardV3">
    ...
    <section className={`partnerOnboarding sellerAccountSetup ${setupApproved ? "approved" : "hasPending"} ${setupCollapsed ? "collapsed" : ""}`}><div className="setupMain"><div className="setupHeading"><div><h3>{setupCollapsed ? (setupApproved ? "Account approved" : `Account setup pending at step ${setupPendingIndex + 1}`) : "Account Setup"}</h3>{!setupCollapsed && <p>Complete each step to activate all seller benefits.</p>}</div></div>{!setupCollapsed && <div className="onboardingSteps">{setupSteps.map(([label, complete], index) => <button type="button" key={label} className={`onboardingStep ${complete ? "completed" : "pending"}`} onClick={() => onNavigate(label === "First product" ? "products" : label === "Bank details" ? "bank" : label === "KYC verification" ? "kyc" : "profile")}><span>{index + 1}</span><div><strong>{label}</strong><small>{complete ? "Completed" : "Pending"}</small></div></button>)}</div>}</div><div className="setupActions">{!setupCollapsed && <div className="setupShield">{setupApproved ? <ShieldCheck /> : <><ShieldAlert /><b>{Math.round((setupSteps.filter(([, complete]) => complete).length / setupSteps.length) * 100)}%</b></>}</div>}<button type="button" className="setupMinimize" onClick={() => setSetupCollapsed((value) => !value)} aria-label={setupCollapsed ? "Expand account setup" : "Minimize account setup"}>{setupCollapsed ? <ChevronDown /> : <Minus />}</button></div></section>
