...
import { Award, BadgeIndianRupee, BarChart3, Bell, Building2, Boxes, Check, CircleDollarSign, Eye, EyeOff, FileCheck2, Gift, Headphones, KeyRound, LayoutDashboard, LockKeyhole, LogOut, Megaphone, MoreVertical, PackageCheck, Printer, Search, ShieldCheck, ShoppingCart, Star, Store, TrendingUp, Truck, UserRound, Users, WalletCards, X } from "lucide-react";
...
const blankRegistration = { companyName: "", businessName: "", address: "", city: "", state: "", gstState: "", businessState: "", pinCode: "", mobile: "", email: "", isGstRegistered: false, gstNumber: "", gstCertificate: "", declarationAccepted: false, referralSellerId: referralFromHash() };
...
const sellerMenuRoutes = new Set(["dashboard", "profile", "products", "orders", "wallet", "kyc", "bank", "password"]);
  ...
  return <>
    <PortalAuthCard portal="Seller" heading="Register as a seller" subtitle="Create your seller account to start listing products." dividerText="Seller registration" pageClassName="partnerRegistrationPage sellerRegistrationPage" panelClassName="partnerRegistrationPanel" onBack={onBack} settings={settings}>
      ...
      <form className="authForm formGrid twoColumn partnerRegistrationForm" onSubmit={onSubmit}>
        ...
        <label className="full">Referral Seller ID (optional)<input pattern="HRS\d{6}" maxLength="9" placeholder="HRS123456" value={registration.referralSellerId} onChange={(event) => update("referralSellerId", event.target.value.toUpperCase().replace(/[^A-Z0-9]/g, "").slice(0, 9))} /><small>Enter the Seller ID of the person who referred you.</small></label>
...
function SellerDashboard({ data }) {
  ...
  return <div className="sellerDashboardV2">
    ...
    <section className="panel referralPanel"><h3>Refer another seller</h3><p>{data.referralCount || 0} seller(s) joined with your Seller ID.</p><div className="referralUrl"><input readOnly value={`${window.location.origin}/${data.referralLink || "#/seller/register"}`} /><button className="primaryButton" type="button" onClick={() => navigator.clipboard.writeText(`${window.location.origin}/${data.referralLink || "#/seller/register"}`)}>Copy link</button></div></section>
...
function SellerOrders({ orders, update, action, shippingMode }) {
  const statuses = ["Accepted", "Processing", "Packed", "Shipped", "Delivered", "Cancelled"];
  ...
  return <section className="sellerOrdersPage">
    ...
    <div className="panel tableWrap"><table><thead><tr><th>Order</th><th>Customer</th><th>Product</th><th>Qty</th><th>Amount</th><th>Item status</th><th>Actions</th></tr></thead><tbody>{filtered.flatMap((order) => order.items.map((item, index) => <tr key={`${order._id}-${item.product}`}><td><strong>{order.orderNumber}</strong><br />{new Date(order.createdAt).toLocaleDateString("en-IN")}</td><td>{order.customer?.name || order.address?.name}<br /><small>{order.customer?.email || order.address?.email}</small></td><td>{item.name}<br /><small>SKU: {item.sku}</small></td><td>{item.quantity}</td><td>{money(item.price * item.quantity)}</td><td><button className={`sellerStatusButton ${String(item.sellerStatus).toLowerCase()}`} type="button" onClick={() => setStatusDialog({ order, item, status: item.sellerStatus || "Pending", note: "" })}>{item.sellerStatus || "Pending"} ▾</button></td><td>{index === 0 && <div className="sellerOrderMenu"><button type="button" aria-label="Order actions" onClick={() => setMenu(menu === order._id ? "" : order._id)}><MoreVertical size={18} /></button>{menu === order._id && <div><button type="button" onClick={() => { setMenu(""); order.invoiceNumber ? printSellerDocument(order, "invoice") : action("invoice", order); }}>Print invoice</button><button type="button" onClick={() => { setMenu(""); printSellerDocument(order, "packing"); }}>Print packing slip</button>{shippingMode === "shiprocket" && <button type="button" onClick={() => { setMenu(""); action("shiprocket", order); }}>Send to ShipRocket</button>}</div>}</div>}</td></tr>))}{!filtered.length && <tr><td colSpan="7">No orders match these filters.</td></tr>}</tbody></table></div>
    {statusDialog && <div className="modalOverlay" role="dialog" aria-modal="true"><form className="sellerStatusModal" onSubmit={(event) => { event.preventDefault(); update(statusDialog.order._id, statusDialog.item.product, { status: statusDialog.status, note: statusDialog.note }); setStatusDialog(null); }}><div className="panelHeader"><div><span className="eyebrow">Verify status change</span><h2>{statusDialog.order.orderNumber}</h2></div><button className="inlineButton" type="button" onClick={() => setStatusDialog(null)}>Close</button></div><label>New item status<select value={statusDialog.status} onChange={(event) => setStatusDialog({ ...statusDialog, status: event.target.value })}>{statuses.map((status) => <option key={status}>{status}</option>)}</select></label><label>Verification notes<textarea required value={statusDialog.note} onChange={(event) => setStatusDialog({ ...statusDialog, note: event.target.value })} placeholder="Add notes about this fulfillment step..." /></label><button className="primaryButton">Verify and update status</button></form></div>}
  </section>;
}
