...
import { ArrowRight, Award, BadgeIndianRupee, BarChart3, Bell, Building2, Boxes, CalendarDays, Check, ChevronDown, CircleDollarSign, Eye, EyeOff, FileCheck2, Gift, Headphones, ImagePlus, KeyRound, LayoutDashboard, LockKeyhole, LogOut, Mail, Menu, Megaphone, Minus, MoreVertical, PackageCheck, Printer, Search, ShieldAlert, ShieldCheck, ShoppingCart, Star, Store, TrendingUp, Truck, UserRound, Users, WalletCards, X } from "lucide-react";
...
const printSellerDocument = (order, type) => {
  ...
  if (!packing) { openInvoice(order, true); return; }
  ...
  popup.document.write("<style>.addresses>div:first-child{display:none}.addresses>div{width:50%}</style>");
...
const sellerLocationRoute = () => {
  if (window.location.hash.startsWith("#/seller")) return window.location.hash.split("?")[0];
...
const referralFromHash = () => new URLSearchParams(window.location.hash.split("?")[1] || window.location.search).get("ref")?.trim().toUpperCase() || "";
const blankRegistration = { name: "", companyName: "", businessName: "", address: "", city: "", state: "", gstState: "", businessState: "", pinCode: "", pickupSameAsBusiness: true, pickupAddress: "", pickupCity: "", pickupState: "", pickupPinCode: "", mobile: "", email: "", isGstRegistered: false, gstNumber: "", gstCertificate: "", declarationAccepted: false, referralSellerId: referralFromHash() };
...
const sellerMenuRoutes = new Set(["dashboard", "profile", "products", "orders", "returns", "wallet", "payouts", "reviews", "kyc", "bank", "support", "password"]);
  ...
  return <>
    <PortalAuthCard portal="Seller" heading="Register as a seller" subtitle="Create your seller account to start listing products." dividerText="Seller registration" pageClassName="partnerRegistrationPage sellerRegistrationPage" panelClassName="partnerRegistrationPanel" onBack={onBack} settings={settings}>
      ...
      <form className="authForm formGrid twoColumn partnerRegistrationForm" onSubmit={onSubmit}>
        ...
        <label className="toggleRow full"><input type="checkbox" checked={registration.pickupSameAsBusiness} onChange={(event) => update("pickupSameAsBusiness", event.target.checked)} /><span>Pickup address is the same as business address</span></label>
        {!registration.pickupSameAsBusiness && <>
          <label className="full">Pickup address<input required value={registration.pickupAddress} onChange={(event) => update("pickupAddress", event.target.value)} /></label>
          <label>Pickup city<input required value={registration.pickupCity} onChange={(event) => update("pickupCity", event.target.value)} /></label>
          <label>Pickup state<input required value={registration.pickupState} onChange={(event) => update("pickupState", event.target.value)} /></label>
          <label>Pickup PIN code<input required inputMode="numeric" value={registration.pickupPinCode} onChange={(event) => update("pickupPinCode", event.target.value.replace(/\D/g, ""))} /></label>
        </>}
...
function SellerProductDetails({ product, onBack, onEdit }) {
  ...
  const detailRows = [["SKU", product.sku], ["Category", product.category?.name], ["HSN Code", product.hsnCode], ["Brand / Manufacturer", product.manufacturerBrand], ["Price", money(product.price)], ["Offer price", money(product.offerPrice || product.price)], ["Tax", product.taxCategory ? `${product.taxCategory.name} (${product.taxCategory.rate}%)` : "None"], ["Stock", product.isStockManageable ? product.stock : "Not managed"], ["Actual weight", product.actualWeight ? `${product.actualWeight} ${product.weightUnit || "kg"}` : ""], ["Volumetric weight", product.volumetricWeight ? `${product.volumetricWeight} kg` : ""], ["Length", product.length ? `${product.length} ${product.dimensionUnit || "cm"}` : ""], ["Width", product.breadth ? `${product.breadth} ${product.dimensionUnit || "cm"}` : ""], ["Height", product.height ? `${product.height} ${product.dimensionUnit || "cm"}` : ""], ["Warranty", product.warranty], ["Approval", product.approvalStatus?.replaceAll("_", " ")], ["Store visibility", product.sellerEnabled ? "Enabled" : "Disabled"]];
...
export default function SellerPortal({ onBack, settings = {} }) {
  ...
  const [screen, setScreen] = useState(seller ? sellerScreenFromHash() : sellerLocationRoute() === "/seller/register" || sellerLocationRoute() === "#/seller/register" ? "register" : "login");
  ...
  useEffect(() => {
    ...
    return () => { button.remove(); sidebarWallet.remove(); };
  }, [seller]);
  useEffect(() => {
  ...
  useEffect(() => {
    const syncSellerScreen = () => {
      ...
      else if (["#/seller", "#/seller/login", "/seller", "/seller/login"].includes(route)) setScreen("login");
    };
  ...
  useEffect(() => {
    ...
    root.style.setProperty("--seller-header-avatar", profileImage ? `url(${JSON.stringify(profileImage)})` : "url('/images/e-commerce/account/avatar.svg')");
    root.style.setProperty("--seller-header-wallet", JSON.stringify(`Wallet Balance  ${money(data.wallet.walletBalance)}`));
    root.style.setProperty("--seller-header-id", JSON.stringify(seller?.sellerNumber || "Seller"));
    return () => {
  ...
  if (!seller && screen === "login") return <SellerLoginScreen settings={settings} onBack={onBack} message={message} login={login} setLogin={setLogin} busy={busy} onSubmit={signIn} onForgot={() => setScreen("forgot")} onSignup={() => { navigateSellerPath("/seller/register"); setMessage(""); }} />;
  ...
  if (!seller && screen === "register") return <SellerRegistrationScreen settings={settings} onBack={onBack} onLogin={() => { navigateSellerPath("/seller/login"); setMessage(""); }} registration={registration} setRegistration={setRegistration} registrationOtp={registrationOtp} setRegistrationOtp={setRegistrationOtp} message={message} setMessage={setMessage} busy={busy} onSubmit={register} onVerify={verifyRegistrationOtp} />;
...
function SellerDashboard({ data }) {
  ...
  const statuses = ["Completed", "Delivered", "Shipped", "Processing", "Pending", "Cancelled", "Returned"];
  ...
  const deliveredPercent = Math.round(((statusCounts.Delivered + statusCounts.Completed) / totalStatusItems) * 100);
  ...
  const setupSteps = [["Registration", Boolean(seller.companyName && seller.registeredAt)], ["Shipping preference", Boolean(seller.shippingMode)], ["Bank details", Boolean(seller.bankDetails?.accountNumber)], ["KYC verification", seller.kyc?.pan?.status === "approved"]];
  ...
  return <div className="sellerDashboardV3">
    ...
    <section className={`partnerOnboarding sellerAccountSetup ${setupApproved ? "approved completedSetup" : "hasPending"} ${setupCollapsed ? "collapsed" : ""}`}><div className="setupMain"><div className="setupHeading"><div><h3>{setupApproved ? "Account setup complete" : setupCollapsed ? `Account setup pending at step ${setupPendingIndex + 1}` : "Account Setup"}</h3>{!setupCollapsed && <p>{setupApproved ? "All account setup steps are complete." : "Complete each step to activate all seller benefits."}</p>}</div></div>{!setupCollapsed && <div className="onboardingSteps">{setupSteps.map(([label, complete], index) => <button type="button" key={label} className={`onboardingStep ${complete ? "completed" : "pending"}`} onClick={() => onNavigate(label === "Bank details" ? "bank" : label === "KYC verification" ? "kyc" : "profile")}><span>{index + 1}</span><div><strong>{label}</strong><small>{complete ? "Completed" : "Pending"}</small></div></button>)}</div>}</div><div className="setupActions"><div className="setupShield">{setupApproved ? <ShieldCheck /> : <ShieldAlert />}<b>{Math.round((setupSteps.filter(([, complete]) => complete).length / setupSteps.length) * 100)}%</b></div>{!setupApproved && <button type="button" className="setupMinimize" onClick={() => setSetupCollapsed((value) => !value)} aria-label={setupCollapsed ? "Expand account setup" : "Minimize account setup"}>{setupCollapsed ? <ChevronDown /> : <Minus />}</button>}</div></section>
    ...
    <section className="sellerReferenceOrders dashboardCard"><header><h3>Recent Orders</h3><button type="button" onClick={() => onNavigate("orders")}>View All →</button></header><div className="sellerRecentOrderList">{orders.slice(0, 5).map((order, index) => { const sellerItems = order.items || []; const status = sellerItems[0]?.sellerStatus || order.status; const customer = order.customer?.name || order.address?.name || "Customer"; return <article key={order._id}><div className="recentOrderId"><strong>{order.orderNumber}</strong><small>{new Date(order.createdAt).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" })}</small></div><span className={`recentCustomerAvatar tone${index % 4}`}>{customer.slice(0, 1)}</span><div className="recentCustomer"><strong>{customer}</strong><small>{[order.address?.city, order.address?.state].filter(Boolean).join(", ")}</small></div><div className="recentOrderAmount"><strong>{money(sellerItems.reduce((sum, item) => sum + item.price * item.quantity, 0))}</strong><small>{order.payment?.provider === "cod" ? "COD" : "Prepaid"}</small></div><span className={`orderBadge ${String(status).toLowerCase().replaceAll(" ", "-")}`}>{status}</span><MoreVertical size={18} /></article>; })}{!orders.length && <p className="emptyDashboardData">Your latest orders will appear here.</p>}</div></section>
...
function SellerProfile({ seller, save }) {
  ...
  const [form, setForm] = useState({ companyName: seller.companyName, address: seller.address, city: seller.city, state: seller.state, pinCode: seller.pinCode, pickupSameAsBusiness: seller.pickupSameAsBusiness !== false, pickupAddress: seller.pickupAddress || seller.address, pickupCity: seller.pickupCity || seller.city, pickupState: seller.pickupState || seller.state, pickupPinCode: seller.pickupPinCode || seller.pinCode, mobile: seller.mobile, profileImage: seller.profileImage || "", shippingMode: seller.shippingMode || "shiprocket" });
  ...
  return <section className="sellerProfilePage">
    <div className="sellerProfileHeading"><div><span>Dashboard　›　Profile</span><h2 className={`sellerVerifiedName ${locked ? "approved" : "pending"}`}>{seller.companyName} {locked ? <ShieldCheck size={22} /> : <ShieldAlert size={22} />}</h2><p>Seller Profile · <strong>{locked ? "Approved" : "Approval pending"}</strong></p></div><span className={`sellerApprovalBadge ${locked ? "approved" : "pending"}`}>{locked ? "Approved seller" : "Not approved yet"}</span></div>
    ...
    <form onSubmit={(event) => { event.preventDefault(); save(locked ? { profileImage: form.profileImage, shippingMode: form.shippingMode } : form); }}>
      <aside className="panel sellerProfileIdentity">
        <div className={`sellerProfilePhoto ${locked ? "approved" : "pending"}`}>{form.profileImage ? <img src={form.profileImage} alt={seller.companyName} /> : <span>{seller.companyName?.[0]}</span>}<i>{locked ? <ShieldCheck size={19} /> : <ShieldAlert size={19} />}</i><label title="Change profile picture">✎<input hidden type="file" accept="image/*" onChange={(event) => uploadPhoto(event.target.files?.[0])} /></label></div>
        <h3 className={`sellerIdentityApproval ${locked ? "approved" : "pending"}`}>{seller.companyName} {locked ? <ShieldCheck size={17} /> : <ShieldAlert size={17} />}</h3><p>{seller.email}</p><strong>{seller.sellerNumber}</strong>
        <dl><div><dt>Account status</dt><dd className={seller.status === "active" ? "approved" : "pending"}>{seller.status || "pending"}</dd></div><div><dt>Approval status</dt><dd className={locked ? "approved" : "pending"}>{seller.approvalStatus || "pending"}</dd></div><div><dt>Commission</dt><dd>{seller.commissionRate}%</dd></div></dl>
      </aside>
      <div className="sellerProfileForms">
        ...
        <section className="panel"><div className="sellerProfileSectionTitle"><div><h3>Pickup Address</h3><p>Address used to collect seller shipments.</p></div><Truck /></div><div className="formGrid twoColumn"><label className="toggleRow full"><input type="checkbox" disabled={locked} checked={form.pickupSameAsBusiness} onChange={(event) => update("pickupSameAsBusiness", event.target.checked)} /><span>Pickup address is the same as business address</span></label>{!form.pickupSameAsBusiness && <><label className="full">Street address<input required disabled={locked} value={form.pickupAddress} onChange={(event) => update("pickupAddress", event.target.value)} /></label><label>City<input required disabled={locked} value={form.pickupCity} onChange={(event) => update("pickupCity", event.target.value)} /></label><label>State<input required disabled={locked} value={form.pickupState} onChange={(event) => update("pickupState", event.target.value)} /></label><label>PIN code<input required disabled={locked} value={form.pickupPinCode} onChange={(event) => update("pickupPinCode", event.target.value.replace(/\D/g, ""))} /></label></>}</div></section>
...
function SellerOrderDetails({ order, tab, setTab, onClose }) {
  ...
  return <div className="modalOverlay" role="dialog" aria-modal="true"><section className="orderDetailModal"><div className="panelHeader"><div><span className="eyebrow">Seller order details</span><h2>{order.orderNumber}</h2></div><button className="inlineButton" type="button" onClick={onClose}>Close</button></div><nav><button className={tab === "summary" ? "active" : ""} onClick={() => setTab("summary")}>Order Items &amp; Summary</button><button className={tab === "parties" ? "active" : ""} onClick={() => setTab("parties")}>Seller &amp; Customer Details</button><button className={tab === "status" ? "active" : ""} onClick={() => setTab("status")}>Item Status</button></nav>{tab === "summary" ? <div className="orderDetailSummary"><div className="orderDetailMeta"><span><strong>Order dated</strong>{new Date(order.createdAt).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" })}</span><span><strong>Last status</strong>{latestStatus?.status || order.status || "Pending"}</span></div><table><thead><tr><th>Product</th><th>SKU</th><th>Qty</th><th>Amount</th></tr></thead><tbody>{order.items.map((item) => { const image = item.productDetails?.imageVariants?.storefront || item.productDetails?.mainImage; return <tr key={`${item.product}-${item.sku}`}><td><div className="orderProductCell">{image ? <img src={image} alt="" /> : <span className="orderProductImageMissing">No image</span>}<a href={storefrontProductUrl(item.product)} target="_blank" rel="noreferrer">{item.name}</a></div></td><td>{item.sku}</td><td>{item.quantity}</td><td>{money(item.price * item.quantity)}</td></tr>; })}</tbody></table><dl><div><dt>Items amount</dt><dd>{money(Number(order.grandTotal || 0) - (Number(order.shipping?.amount) || Number(order.shippingTotal)))}</dd></div><div><dt>Shipping</dt><dd>{money(Number(order.shipping?.amount) || Number(order.shippingTotal))}</dd></div><div><dt>Total</dt><dd>{money(order.grandTotal)}</dd></div></dl></div> : tab === "parties" ? <div className="orderPartyGrid"><section><h3>Customer</h3><p><strong>{order.customer?.name || order.address?.name || "Guest"}</strong><br />{order.customer?.email || order.address?.email}<br />{order.customer?.phone || order.address?.phone}<br />{order.address?.shippingAddress || order.address?.billingAddress}<br />{[order.address?.city, order.address?.state, order.address?.postalCode].filter(Boolean).join(", ")}</p></section>{sellers.map((seller) => <section key={seller._id || seller.sellerNumber}><h3>Seller</h3><p><strong>{seller.companyName}</strong><br />Seller ID: {seller.sellerNumber}<br />{seller.email}<br />{seller.mobile}<br />{[seller.address, seller.city, seller.state, seller.pinCode].filter(Boolean).join(", ")}</p></section>)}</div> : <div className="orderStatusHistory">{timeline.length ? timeline.map((entry, index) => <article key={entry._id || `${entry.createdAt}-${index}`}><span className={`sellerStatusButton ${String(entry.status || "pending").toLowerCase().replaceAll(" ", "-")}`}>{entry.status || "Update"}</span><div><strong>{entry.title}</strong><small>{new Date(entry.createdAt).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" })}</small>{entry.comment && <p>{entry.comment}</p>}{entry.details && <small>{entry.details}</small>}</div></article>) : <p>No item status updates have been recorded.</p>}</div>}</section></div>;
}

function ManualCourierDialog({ dialog, setDialog, onSaved }) {
  if (!dialog) return null;
  return <div className="modalOverlay" role="dialog" aria-modal="true"><form className="sellerStatusModal" onSubmit={async (event) => { event.preventDefault(); setDialog((current) => ({ ...current, busy: true, error: "" })); try { const updated = await api.saveSellerManualCourier(dialog.order._id, { courierName: dialog.courierName, trackingId: dialog.trackingId, trackingUrl: dialog.trackingUrl }); onSaved(updated); } catch (error) { setDialog((current) => ({ ...current, busy: false, error: error.message })); } }}><div className="panelHeader"><div><span className="eyebrow">Manual fulfillment</span><h2>{dialog.order.orderNumber}</h2></div><button className="inlineButton" type="button" disabled={dialog.busy} onClick={() => setDialog(null)}>Close</button></div><p className="mutedText">Enter the courier details that the customer will use to track this order.</p><label>Courier name<input required value={dialog.courierName} onChange={(event) => setDialog({ ...dialog, courierName: event.target.value })} placeholder="Example: Blue Dart" /></label><label>Tracking ID<input required value={dialog.trackingId} onChange={(event) => setDialog({ ...dialog, trackingId: event.target.value })} placeholder="Courier AWB or tracking number" /></label><label>Tracking URL<input required type="url" value={dialog.trackingUrl} onChange={(event) => setDialog({ ...dialog, trackingUrl: event.target.value })} placeholder="https://courier.example/track/..." /></label>{dialog.error && <p className="errorText">{dialog.error}</p>}<button className="primaryButton" disabled={dialog.busy}>{dialog.busy ? "Saving…" : "Save courier details & mark shipped"}</button></form></div>;
}

const sellerReturnLabel = (status) => ({ Requested: "Pending", Approved: "Accepted", "Pickup Arranged": "In Transit", Received: "Received", Closed: "Refund Issued", Rejected: "Rejected" })[status] || status || "Pending";
function SellerReturnsReadOnly({ orders }) {
  const rows = orders.flatMap((order) => (order.items || []).filter((item) => item.returnRequest?.status).map((item) => ({ order, item })));
  return <section className="sellerOrdersPage sellerReturnsReadOnly"><div className="sellerKycHeading"><div><span className="eyebrow">Admin-managed returns</span><h2>Returns &amp; Refunds</h2><p>Customer return items and their current status. Return processing is controlled by Admin.</p></div></div><div className="panel tableWrap"><table><thead><tr><th>Order</th><th>Product</th><th>Customer</th><th>Return reason</th><th>Requested</th><th>Status</th><th>Admin update</th></tr></thead><tbody>{rows.map(({ order, item }) => { const status = sellerReturnLabel(item.returnRequest.status); return <tr key={`${order._id}-${item.product}`}><td><strong>{order.orderNumber}</strong><br/><small>{new Date(order.createdAt).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" })}</small></td><td><strong>{item.name}</strong><br/><small>{item.sku} · Qty {item.quantity}</small></td><td>{order.customer?.name || order.address?.name || "Customer"}</td><td>{item.returnRequest.reason || "—"}<br/><small>{item.returnRequest.comments || ""}</small></td><td>{item.returnRequest.requestedAt ? new Date(item.returnRequest.requestedAt).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" }) : "—"}</td><td><span className={`sellerReturnStatus ${status.toLowerCase().replaceAll(" ", "-")}`}>{status}</span></td><td>{item.returnRequest.reviewNote || "Waiting for Admin update"}</td></tr>; })}{!rows.length && <tr><td colSpan="7">No customer return requests yet.</td></tr>}</tbody></table></div></section>;
}
function SellerOrders({ orders, update, returnUpdate, action, shippingMode }) {
  ...
  const openOrder = (order) => { setSelectedOrder(order); window.location.hash = `#/seller/orders/${order._id}`; };
  useEffect(() => { if (selectedOrder && !window.location.hash.includes(String(selectedOrder._id))) window.location.hash = `#/seller/orders/${selectedOrder._id}`; }, [selectedOrder]);
  useEffect(() => {
    const closeMenu = (event) => { if (!event.target.closest(".sellerOrderMenu")) setMenu(""); };
  ...
  const filtered = orders.filter((order) => {
    ...
    const delivered = order.items.length > 0 && order.items.every((item) => ["Delivered", "Completed"].includes(item.sellerStatus));
    const tabMatch = tab === "delivered" ? delivered : tab === "pending" ? !delivered : false;
    return tabMatch && text.includes(filters.search.toLowerCase()) && (!filters.from || created >= new Date(filters.from)) && (!filters.to || created <= new Date(`${filters.to}T23:59:59`)) && (filters.status === "all" || order.items.some((item) => item.sellerStatus === filters.status));
  });
  const pendingGroups = [...orders.flatMap((order) => order.items.filter((item) => !["Delivered", "Completed", "Cancelled", "Returned"].includes(item.sellerStatus)).map((item) => ({ ...item, orderNumber: order.orderNumber }))).reduce((map, item) => { const key = item.sku; const current = map.get(key) || { sku: item.sku, name: item.name, quantity: 0, orders: new Set(), product: item.product, productDetails: item.productDetails }; current.quantity += item.quantity; current.orders.add(item.orderNumber); map.set(key, current); return map; }, new Map()).values()];
  if (activeSellerPortalScreen === "returns") return <SellerReturnsReadOnly orders={orders} />;
  return <section className="sellerOrdersPage">
    <nav className="orderFulfillmentTabs"><button className={tab === "pending" ? "active" : ""} type="button" onClick={() => setTab("pending")}>Current Pending Orders</button><button className={tab === "grouping" ? "active" : ""} type="button" onClick={() => setTab("grouping")}>Pending Item Grouping</button><button className={tab === "delivered" ? "active" : ""} type="button" onClick={() => setTab("delivered")}>Delivered Orders</button></nav>
    {selectedOrder && <div className="trackingRouteOverlay"><OrderTrackingPage order={selectedOrder} onBack={() => { setSelectedOrder(null); window.location.hash = "#/seller/orders"; }} /></div>}
    {tab === "grouping" && <div className="panel tableWrap sellerPendingGrouping"><table><thead><tr><th>Image</th><th>Product</th><th>SKU</th><th>Qty Required</th><th>Orders</th></tr></thead><tbody>{pendingGroups.map((item) => <tr key={item.sku}><td>{item.productDetails?.mainImage ? <img className="sellerProductThumb" src={item.productDetails.mainImage} alt="" /> : "—"}</td><td><a href={`#/product/${item.product}`} target="_blank" rel="noreferrer">{item.name}</a></td><td>{item.sku}</td><td>{item.quantity}</td><td>{[...item.orders].join(", ")}</td></tr>)}</tbody></table></div>}
    ...
    {shippingMode !== "shiprocket" && tab === "pending" && <><div className="manualCourierActions">{filtered.filter((order) => order.shipping?.syncStatus === "Manual courier details added" || order.items.every((item) => item.sellerStatus === "Ready to Dispatch")).map((order) => <button className="primaryButton" type="button" key={order._id} onClick={() => setManualCourierDialog({ order, courierName: order.shipping?.courierName || "", trackingId: order.shipping?.awbCode || "", trackingUrl: order.shipping?.trackingUrl || "", error: "", busy: false })}><Truck size={16} /> {order.shipping?.awbCode ? `Courier details · ${order.orderNumber}` : `Add courier details · ${order.orderNumber}`}</button>)}</div><ManualCourierDialog dialog={manualCourierDialog} setDialog={setManualCourierDialog} onSaved={(updated) => { Object.assign(manualCourierDialog.order, updated); setFilters((current) => ({ ...current })); setManualCourierDialog(null); }} /></>}
    <div className="panel tableWrap"><table><thead><tr><th>Order</th><th>Customer</th><th>Invoice</th><th>Item status</th><th>Return information</th><th>Actions</th></tr></thead><tbody>{filtered.flatMap((order) => order.items.map((item, index) => { const closes = new Date(new Date(item.deliveredAt || order.updatedAt).getTime() + Number(item.returnDays || 0) * 86400000); const canViewCommission = ["Delivered", "Completed"].includes(item.sellerStatus); return <tr className="clickableTableRow" key={`${order._id}-${item.product}`} onClick={(event) => { if (!event.target.closest("button,a,input,select,textarea")) { setSelectedOrder(order); setDetailTab("summary"); } }}><td><strong>{order.orderNumber}</strong><br />{new Date(order.createdAt).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" })}{order.shipping?.awbCode && <><br /><small>AWB: {order.shipping.awbCode}</small></>}</td><td>{order.customer?.name || order.address?.name}<br /><small>{order.customer?.email || order.address?.email}</small></td>{index === 0 && <td rowSpan={order.items.length}><strong>{order.invoiceNumber || "Not generated"}</strong><br /><small>Shipping {money(Number(order.shipping?.amount) || Number(order.shippingTotal))}</small><br /><strong>Total {money(order.grandTotal)}</strong></td>}<td><button className={`sellerStatusButton ${String(item.sellerStatus).toLowerCase().replaceAll(" ", "-")}`} type="button" disabled={item.sellerStatus === "Completed"} onClick={() => setStatusDialog({ order, item, status: item.sellerStatus || "Pending", note: "", statusDate: new Date().toISOString().slice(0, 10) })}>{item.sellerStatus || "Pending"} ▾</button>{item.sellerStatus === "Delivered" && <small>Delivered</small>}</td><td><span className={`returnWindowBadge ${canViewCommission && closes <= new Date() ? "closed" : "open"}`}>{!canViewCommission ? "Starts after delivery" : closes <= new Date() ? "Return window closed" : `Open until ${closes.toLocaleDateString("en-IN")}`}</span></td><td>{index === 0 && <div className="sellerOrderMenu"><button type="button" aria-label="Order actions" onClick={() => setMenu(menu === order._id ? "" : order._id)}><MoreVertical size={18} /></button>{menu === order._id && <div><button type="button" onClick={() => { setSelectedOrder(order); setDetailTab("summary"); setMenu(""); }}>View order details</button><button type="button" onClick={() => { setMenu(""); order.invoiceNumber ? printSellerDocument(order, "invoice") : action("invoice", order); }}>Print invoice</button><button type="button" onClick={() => { setMenu(""); printSellerDocument(order, "packing"); }}>Print packing slip</button>{shippingMode === "shiprocket" && <button type="button" disabled={order.items.some((entry) => entry.sellerStatus !== "Ready to Dispatch")} onClick={() => { setMenu(""); action("shiprocket", order); }}>Send packet to ShipRocket</button>}</div>}</div>}{canViewCommission && <button className="inlineButton" type="button" onClick={async () => { const result = item.settlement?.settledAt ? { payout: { ...item.settlement, commissionAmount: item.settlement.platformFee } } : await api.settleSellerOrderItem(order._id, item.product); setSettlement({ order, item, ...result.payout, pending: Boolean(result.pending), returnWindowClosesAt: result.returnWindowClosesAt }); }}>Commission</button>}{item.returnRequest?.status && <span className={`sellerReturnStatus ${sellerReturnLabel(item.returnRequest.status).toLowerCase().replaceAll(" ", "-")}`}>{sellerReturnLabel(item.returnRequest.status)}</span>}</td></tr>; }))}{!filtered.length && <tr><td colSpan="6">No orders match these filters.</td></tr>}</tbody></table></div>
    {settlement && <div className="modalOverlay" role="dialog" aria-modal="true"><section className="sellerStatusModal settlementDialog"><div className="panelHeader"><div><span className="eyebrow">Order Settlement Details</span><h2>{settlement.order.orderNumber}</h2></div><button className="inlineButton" type="button" onClick={() => setSettlement(null)}>Close</button></div>{settlement.pending && <div className="notice">Commission calculated. It will be credited to your wallet automatically after the return window closes on {new Date(settlement.returnWindowClosesAt).toLocaleDateString("en-IN")}.</div>}<p><strong>{settlement.item.name}</strong> · Qty {settlement.item.quantity}</p><dl><div><dt>Total Order Amount</dt><dd>{money(settlement.grossAmount)}</dd></div><div><dt>Platform Fee ({settlement.commissionRate}%)</dt><dd>− {money(settlement.commissionAmount)}</dd></div><div><dt>Payment Gateway Fee ({settlement.paymentGatewayFeeRate}%)</dt><dd>− {money(settlement.paymentGatewayFee)}</dd></div><div><dt>Shipping Charge ({settlement.shippingPaidBy})</dt><dd>{settlement.shippingPaidBy === "seller" ? "− " : ""}{money(settlement.shippingCharge)}</dd></div><div><dt>GST on Commission</dt><dd>− {money(settlement.gstOnCommission)}</dd></div><div><dt>Other Charges</dt><dd>− {money(settlement.otherCharges)}</dd></div></dl><div className="settlementNet"><span>Net Settlement Amount</span><strong>{money(settlement.netAmount)}</strong></div></section></div>}
...
const transactionDate = (value) => new Date(value).toLocaleString("en-IN", { day: "2-digit", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit", hour12: true });
function SellerTransactionHistory() {
  const [result, setResult] = useState({ items: [], pagination: { page: 1, limit: 10, total: 0 }, summary: {} });
  const [filters, setFilters] = useState({ page: 1, limit: 10, period: "all", type: "All", status: "All", q: "", from: "", to: "" });
  const [showAll, setShowAll] = useState(false); const [selected, setSelected] = useState(null); const [loading, setLoading] = useState(true);
  useEffect(() => { let active = true; setLoading(true); const timer = window.setTimeout(() => api.sellerTransactions(filters).then((data) => { if (active) setResult(data); }).catch((error) => showToast(error.message, "error")).finally(() => active && setLoading(false)), 200); return () => { active = false; window.clearTimeout(timer); }; }, [filters]);
  const change = (key, value) => setFilters((current) => ({ ...current, [key]: value, page: key === "page" ? value : 1 }));
  const exportRows = result.items.map((item) => [transactionDate(item.date), item.transactionId, item.orderId, item.description, item.type, Number(item.amount).toFixed(2), item.status]);
  const download = (kind) => {
    if (kind === "pdf") { const popup = window.open("", "_blank"); popup.document.write(`<title>Transaction Statement</title><h2>Transaction Statement</h2><table border="1" cellspacing="0" cellpadding="8"><tr><th>Date</th><th>Transaction ID</th><th>Order ID</th><th>Description</th><th>Type</th><th>Amount</th><th>Status</th></tr>${exportRows.map((row) => `<tr>${row.map((cell) => `<td>${String(cell).replaceAll("<", "&lt;")}</td>`).join("")}</tr>`).join("")}</table>`); popup.document.close(); popup.print(); return; }
    const rows = [["Date", "Transaction ID", "Order ID", "Description", "Type", "Amount", "Status"], ...exportRows];
    const content = rows.map((row) => row.map((value) => `"${String(value ?? "").replaceAll('"', '""')}"`).join(",")).join("\n");
    const link = document.createElement("a"); link.href = URL.createObjectURL(new Blob([content], { type: kind === "excel" ? "application/vnd.ms-excel" : "text/csv" })); link.download = `seller-transactions.${kind === "excel" ? "xls" : "csv"}`; link.click(); URL.revokeObjectURL(link.href);
  };
  const summary = [["Available Balance", result.summary.availableBalance], ["Pending Settlement", result.summary.pendingSettlement], ["Total Credit", result.summary.totalCredit], ["Total Debit", result.summary.totalDebit], ["Today's Credit", result.summary.todayCredit], ["Today's Debit", result.summary.todayDebit]];
  return <section className={`sellerTransactions panel ${showAll ? "expanded" : ""}`}>
    <div className="sellerWalletSectionTitle"><h3>Recent Transactions</h3><button type="button" onClick={() => setShowAll((value) => !value)}>{showAll ? "Show Recent" : "View All"}</button></div>
    {showAll && <><div className="transactionSummaryGrid">{summary.map(([label, value], index) => <article key={label} className={index === 2 || index === 4 ? "credit" : index === 3 || index === 5 ? "debit" : ""}><span>{label}</span><strong>{money(value)}</strong></article>)}</div>
      <div className="transactionToolbar"><label className="transactionSearch"><Search size={17}/><input value={filters.q} onChange={(event) => change("q", event.target.value)} placeholder="Order ID, Transaction ID, customer or amount" /></label><select value={filters.period} onChange={(event) => change("period", event.target.value)}><option value="all">All Dates</option><option value="today">Today</option><option value="yesterday">Yesterday</option><option value="week">This Week</option><option value="month">This Month</option><option value="last_month">Last Month</option><option value="custom">Custom Date</option></select><select value={filters.type} onChange={(event) => change("type", event.target.value)}><option>All</option><option>Credit</option><option>Debit</option></select><select value={filters.status} onChange={(event) => change("status", event.target.value)}>{["All", "Pending", "Processing", "Completed", "Success", "Failed", "Rejected", "Cancelled"].map((status) => <option key={status}>{status}</option>)}</select>{filters.period === "custom" && <><input type="date" value={filters.from} onChange={(event) => change("from", event.target.value)}/><input type="date" value={filters.to} onChange={(event) => change("to", event.target.value)}/></>}<div className="transactionExports"><button type="button" onClick={() => download("pdf")}>Export PDF</button><button type="button" onClick={() => download("excel")}>Export Excel</button><button type="button" onClick={() => download("csv")}>Export CSV</button></div></div></>}
    <div className="tableWrap transactionDesktopTable"><table><thead><tr><th>Date</th><th>Description</th><th>Type</th><th>Amount</th><th>Status</th></tr></thead><tbody>{loading ? <tr><td colSpan="5">Loading transactions…</td></tr> : result.items.slice(0, showAll ? result.items.length : 8).map((item) => <tr key={item._id}><td>{transactionDate(item.date)}</td><td><button className="transactionDescription" type="button" onClick={() => setSelected(item)}>{item.description}</button></td><td><span className={`walletTransactionType ${item.type.toLowerCase()}`}>{item.type}</span></td><td className={item.type.toLowerCase()}>{item.type === "Credit" ? "+" : "−"} {money(item.amount)}</td><td><span className={`walletTransactionStatus ${String(item.status).toLowerCase()}`}>{item.status}</span></td></tr>)}{!loading && !result.items.length && <tr><td colSpan="5">No transactions match these filters.</td></tr>}</tbody></table></div>
    <div className="transactionMobileCards">{result.items.slice(0, showAll ? result.items.length : 8).map((item) => <button type="button" key={item._id} onClick={() => setSelected(item)}><span><small>{transactionDate(item.date)}</small><strong>{item.description}</strong></span><span className={item.type.toLowerCase()}><b>{item.type === "Credit" ? "+" : "−"} {money(item.amount)}</b><em className={String(item.status).toLowerCase()}>{item.status}</em></span></button>)}</div>
    {showAll && <TablePagination total={result.pagination.total} page={result.pagination.page} pageSize={result.pagination.limit} pageSizes={[10,25,50,100]} onPageChange={(value) => change("page", value)} onPageSizeChange={(value) => change("limit", value)} />}
    {selected && <div className="modalOverlay" role="dialog" aria-modal="true"><section className="sellerStatusModal transactionDetails"><div className="panelHeader"><div><span className="eyebrow">Transaction Details</span><h2>{selected.transactionId}</h2></div><button type="button" className="inlineButton" onClick={() => setSelected(null)}>Close</button></div><dl>{[["Transaction ID", selected.transactionId], ["Order ID", selected.orderId], ["Seller ID", selected.sellerId], ["Customer Name", selected.customerName], ["Payment Method", selected.paymentMethod], ["Settlement Amount", money(selected.settlementAmount)], ["Platform Fee", money(selected.platformFee)], ["Payment Gateway Charge", money(selected.paymentGatewayCharge)], ["Shipping Charge", money(selected.shippingCharge)], ["Tax", money(selected.tax)], ["Net Amount", money(selected.netAmount)], ["Date & Time", transactionDate(selected.date)], ["Status", selected.status], ["Remarks", selected.remarks], ["Admin Notes", selected.adminNotes || "—"]].map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl></section></div>}
  </section>;
}
