...
export const processWithdrawal = asyncHandler(async (req, res) => { const allowedFrom = req.body.status === "paid" ? "approved" : "pending"; const withdrawal = await Withdrawal.findOne({ _id: req.params.id, status: allowedFrom }); if (!withdrawal) { res.status(404); throw new Error(`Withdrawal must be ${allowedFrom} for this action`); } if (!["approved", "rejected", "paid"].includes(req.body.status)) { res.status(400); throw new Error("Invalid withdrawal status"); } if (req.body.status === "paid" && !String(req.body.adminNote || "").trim()) { res.status(400); throw new Error("Payment comments are required"); } withdrawal.status = req.body.status; withdrawal.adminNote = String(req.body.adminNote || "").trim(); withdrawal.processedAt = new Date(); withdrawal.processedBy = req.user._id; if (req.body.status === "paid") withdrawal.paidAt = req.body.paidAt ? new Date(req.body.paidAt) : new Date(); if (req.body.status === "rejected") await Partner.updateOne({ _id: withdrawal.partner }, { $inc: { walletBalance: withdrawal.amount } }); await withdrawal.save(); res.json(withdrawal); });
